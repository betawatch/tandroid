package com.google.android.gms.internal.play_billing;

import java.util.Arrays;
import java.util.List;

/* compiled from: r8-map-id-e9be2e8928caae39c37b14acc2083317da263a6f1414814df554d3ad0d46aba8 */
/* loaded from: classes.dex */
public abstract class u2 {
    public static final t1 a;

    static {
        q2 q2Var = q2.c;
        a = new t1(7);
    }

    public static void a(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        int i11 = 0;
        if (!(list instanceof w1)) {
            if (!z4) {
                while (i11 < list.size()) {
                    int intValue = ((Integer) list.get(i11)).intValue();
                    m1Var.n(i10, (intValue >> 31) ^ (intValue + intValue));
                    i11++;
                }
                return;
            }
            m1Var.m(i10, 2);
            int i12 = 0;
            for (int i13 = 0; i13 < list.size(); i13++) {
                int intValue2 = ((Integer) list.get(i13)).intValue();
                i12 += m1.b((intValue2 >> 31) ^ (intValue2 + intValue2));
            }
            m1Var.o(i12);
            while (i11 < list.size()) {
                int intValue3 = ((Integer) list.get(i11)).intValue();
                m1Var.o((intValue3 >> 31) ^ (intValue3 + intValue3));
                i11++;
            }
            return;
        }
        w1 w1Var = (w1) list;
        if (!z4) {
            while (i11 < w1Var.c) {
                int n10 = w1Var.n(i11);
                m1Var.n(i10, (n10 >> 31) ^ (n10 + n10));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < w1Var.c; i15++) {
            int n11 = w1Var.n(i15);
            i14 += m1.b((n11 >> 31) ^ (n11 + n11));
        }
        m1Var.o(i14);
        while (i11 < w1Var.c) {
            int n12 = w1Var.n(i11);
            m1Var.o((n12 >> 31) ^ (n12 + n12));
            i11++;
        }
    }

    public static void b(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof h2) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                long longValue = ((Long) list.get(i11)).longValue();
                m1Var.p(i10, (longValue >> 63) ^ (longValue + longValue));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            long longValue2 = ((Long) list.get(i13)).longValue();
            i12 += m1.c((longValue2 >> 63) ^ (longValue2 + longValue2));
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            long longValue3 = ((Long) list.get(i11)).longValue();
            m1Var.q((longValue3 >> 63) ^ (longValue3 + longValue3));
            i11++;
        }
    }

    public static void c(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        int i11 = 0;
        if (!(list instanceof w1)) {
            if (!z4) {
                while (i11 < list.size()) {
                    m1Var.n(i10, ((Integer) list.get(i11)).intValue());
                    i11++;
                }
                return;
            }
            m1Var.m(i10, 2);
            int i12 = 0;
            for (int i13 = 0; i13 < list.size(); i13++) {
                i12 += m1.b(((Integer) list.get(i13)).intValue());
            }
            m1Var.o(i12);
            while (i11 < list.size()) {
                m1Var.o(((Integer) list.get(i11)).intValue());
                i11++;
            }
            return;
        }
        w1 w1Var = (w1) list;
        if (!z4) {
            while (i11 < w1Var.c) {
                m1Var.n(i10, w1Var.n(i11));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < w1Var.c; i15++) {
            i14 += m1.b(w1Var.n(i15));
        }
        m1Var.o(i14);
        while (i11 < w1Var.c) {
            m1Var.o(w1Var.n(i11));
            i11++;
        }
    }

    public static void d(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof h2) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                m1Var.p(i10, ((Long) list.get(i11)).longValue());
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            i12 += m1.c(((Long) list.get(i13)).longValue());
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            m1Var.q(((Long) list.get(i11)).longValue());
            i11++;
        }
    }

    public static boolean e(Object obj, Object obj2) {
        if (obj != obj2) {
            return obj != null && obj.equals(obj2);
        }
        return true;
    }

    public static int f(List list) {
        int size = list.size();
        int i10 = 0;
        if (size == 0) {
            return 0;
        }
        if (!(list instanceof w1)) {
            int i11 = 0;
            while (i10 < size) {
                i11 += m1.c(((Integer) list.get(i10)).intValue());
                i10++;
            }
            return i11;
        }
        w1 w1Var = (w1) list;
        int i12 = 0;
        while (i10 < size) {
            i12 += m1.c(w1Var.n(i10));
            i10++;
        }
        return i12;
    }

    public static int g(int i10, List list) {
        int size = list.size();
        if (size == 0) {
            return 0;
        }
        return (m1.b(i10 << 3) + 4) * size;
    }

    public static int h(int i10, List list) {
        int size = list.size();
        if (size == 0) {
            return 0;
        }
        return (m1.b(i10 << 3) + 8) * size;
    }

    public static int i(List list) {
        int size = list.size();
        int i10 = 0;
        if (size == 0) {
            return 0;
        }
        if (!(list instanceof w1)) {
            int i11 = 0;
            while (i10 < size) {
                i11 += m1.c(((Integer) list.get(i10)).intValue());
                i10++;
            }
            return i11;
        }
        w1 w1Var = (w1) list;
        int i12 = 0;
        while (i10 < size) {
            i12 += m1.c(w1Var.n(i10));
            i10++;
        }
        return i12;
    }

    public static int j(List list) {
        int size = list.size();
        if (size == 0) {
            return 0;
        }
        if (list instanceof h2) {
            if (size <= 0) {
                return 0;
            }
            throw null;
        }
        int i10 = 0;
        for (int i11 = 0; i11 < size; i11++) {
            i10 += m1.c(((Long) list.get(i11)).longValue());
        }
        return i10;
    }

    public static int k(List list) {
        int size = list.size();
        int i10 = 0;
        if (size == 0) {
            return 0;
        }
        if (!(list instanceof w1)) {
            int i11 = 0;
            while (i10 < size) {
                int intValue = ((Integer) list.get(i10)).intValue();
                i11 += m1.b((intValue >> 31) ^ (intValue + intValue));
                i10++;
            }
            return i11;
        }
        w1 w1Var = (w1) list;
        int i12 = 0;
        while (i10 < size) {
            int n10 = w1Var.n(i10);
            i12 += m1.b((n10 >> 31) ^ (n10 + n10));
            i10++;
        }
        return i12;
    }

    public static int l(List list) {
        int size = list.size();
        if (size == 0) {
            return 0;
        }
        if (list instanceof h2) {
            if (size <= 0) {
                return 0;
            }
            throw null;
        }
        int i10 = 0;
        for (int i11 = 0; i11 < size; i11++) {
            long longValue = ((Long) list.get(i11)).longValue();
            i10 += m1.c((longValue >> 63) ^ (longValue + longValue));
        }
        return i10;
    }

    public static int m(List list) {
        int size = list.size();
        int i10 = 0;
        if (size == 0) {
            return 0;
        }
        if (!(list instanceof w1)) {
            int i11 = 0;
            while (i10 < size) {
                i11 += m1.b(((Integer) list.get(i10)).intValue());
                i10++;
            }
            return i11;
        }
        w1 w1Var = (w1) list;
        int i12 = 0;
        while (i10 < size) {
            i12 += m1.b(w1Var.n(i10));
            i10++;
        }
        return i12;
    }

    public static int n(List list) {
        int size = list.size();
        if (size == 0) {
            return 0;
        }
        if (list instanceof h2) {
            if (size <= 0) {
                return 0;
            }
            throw null;
        }
        int i10 = 0;
        for (int i11 = 0; i11 < size; i11++) {
            i10 += m1.c(((Long) list.get(i11)).longValue());
        }
        return i10;
    }

    public static Object o(Object obj, int i10, int i11, Object obj2, t1 t1Var) {
        Object obj3 = obj2;
        if (obj2 == null) {
            t1Var.getClass();
            v1 v1Var = (v1) obj;
            x2 x2Var = v1Var.zzc;
            obj3 = x2Var;
            if (x2Var == x2.f) {
                x2 b10 = x2.b();
                v1Var.zzc = b10;
                obj3 = b10;
            }
        }
        ((x2) obj3).c(i10 << 3, Long.valueOf(i11));
        return obj3;
    }

    public static void p(Object obj, Object obj2) {
        v1 v1Var = (v1) obj;
        x2 x2Var = v1Var.zzc;
        x2 x2Var2 = ((v1) obj2).zzc;
        x2 x2Var3 = x2.f;
        if (!x2Var3.equals(x2Var2)) {
            if (x2Var3.equals(x2Var)) {
                int i10 = x2Var.a + x2Var2.a;
                int[] copyOf = Arrays.copyOf(x2Var.b, i10);
                System.arraycopy(x2Var2.b, 0, copyOf, x2Var.a, x2Var2.a);
                Object[] copyOf2 = Arrays.copyOf(x2Var.c, i10);
                System.arraycopy(x2Var2.c, 0, copyOf2, x2Var.a, x2Var2.a);
                x2Var = new x2(i10, copyOf, copyOf2, true);
            } else {
                x2Var.getClass();
                if (!x2Var2.equals(x2Var3)) {
                    if (!x2Var.e) {
                        throw new UnsupportedOperationException();
                    }
                    int i11 = x2Var.a + x2Var2.a;
                    x2Var.e(i11);
                    System.arraycopy(x2Var2.b, 0, x2Var.b, x2Var.a, x2Var2.a);
                    System.arraycopy(x2Var2.c, 0, x2Var.c, x2Var.a, x2Var2.a);
                    x2Var.a = i11;
                }
            }
        }
        v1Var.zzc = x2Var;
    }

    public static void q(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof i1) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                byte booleanValue = ((Boolean) list.get(i11)).booleanValue();
                m1Var.o(i10 << 3);
                int i12 = m1Var.d;
                try {
                    int i13 = i12 + 1;
                    try {
                        m1Var.b[i12] = booleanValue;
                        m1Var.d = i13;
                        i11++;
                    } catch (IndexOutOfBoundsException e6) {
                        e = e6;
                        i12 = i13;
                        throw new b7.y0(i12, m1Var.c, 1, e);
                    }
                } catch (IndexOutOfBoundsException e10) {
                    e = e10;
                }
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < list.size(); i15++) {
            ((Boolean) list.get(i15)).getClass();
            i14++;
        }
        m1Var.o(i14);
        while (i11 < list.size()) {
            byte booleanValue2 = ((Boolean) list.get(i11)).booleanValue();
            int i16 = m1Var.d;
            try {
                int i17 = i16 + 1;
                try {
                    m1Var.b[i16] = booleanValue2;
                    m1Var.d = i17;
                    i11++;
                } catch (IndexOutOfBoundsException e11) {
                    e = e11;
                    i16 = i17;
                    throw new b7.y0(i16, m1Var.c, 1, e);
                }
            } catch (IndexOutOfBoundsException e12) {
                e = e12;
            }
        }
    }

    public static void r(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof n1) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                m1Var.h(i10, Double.doubleToRawLongBits(((Double) list.get(i11)).doubleValue()));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            ((Double) list.get(i13)).getClass();
            i12 += 8;
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            m1Var.i(Double.doubleToRawLongBits(((Double) list.get(i11)).doubleValue()));
            i11++;
        }
    }

    public static void s(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        int i11 = 0;
        if (!(list instanceof w1)) {
            if (!z4) {
                while (i11 < list.size()) {
                    m1Var.j(i10, ((Integer) list.get(i11)).intValue());
                    i11++;
                }
                return;
            }
            m1Var.m(i10, 2);
            int i12 = 0;
            for (int i13 = 0; i13 < list.size(); i13++) {
                i12 += m1.c(((Integer) list.get(i13)).intValue());
            }
            m1Var.o(i12);
            while (i11 < list.size()) {
                m1Var.k(((Integer) list.get(i11)).intValue());
                i11++;
            }
            return;
        }
        w1 w1Var = (w1) list;
        if (!z4) {
            while (i11 < w1Var.c) {
                m1Var.j(i10, w1Var.n(i11));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < w1Var.c; i15++) {
            i14 += m1.c(w1Var.n(i15));
        }
        m1Var.o(i14);
        while (i11 < w1Var.c) {
            m1Var.k(w1Var.n(i11));
            i11++;
        }
    }

    public static void t(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        int i11 = 0;
        if (!(list instanceof w1)) {
            if (!z4) {
                while (i11 < list.size()) {
                    m1Var.f(i10, ((Integer) list.get(i11)).intValue());
                    i11++;
                }
                return;
            }
            m1Var.m(i10, 2);
            int i12 = 0;
            for (int i13 = 0; i13 < list.size(); i13++) {
                ((Integer) list.get(i13)).getClass();
                i12 += 4;
            }
            m1Var.o(i12);
            while (i11 < list.size()) {
                m1Var.g(((Integer) list.get(i11)).intValue());
                i11++;
            }
            return;
        }
        w1 w1Var = (w1) list;
        if (!z4) {
            while (i11 < w1Var.c) {
                m1Var.f(i10, w1Var.n(i11));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < w1Var.c; i15++) {
            w1Var.n(i15);
            i14 += 4;
        }
        m1Var.o(i14);
        while (i11 < w1Var.c) {
            m1Var.g(w1Var.n(i11));
            i11++;
        }
    }

    public static void u(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof h2) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                m1Var.h(i10, ((Long) list.get(i11)).longValue());
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            ((Long) list.get(i13)).getClass();
            i12 += 8;
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            m1Var.i(((Long) list.get(i11)).longValue());
            i11++;
        }
    }

    public static void v(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof r1) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                m1Var.f(i10, Float.floatToRawIntBits(((Float) list.get(i11)).floatValue()));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            ((Float) list.get(i13)).getClass();
            i12 += 4;
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            m1Var.g(Float.floatToRawIntBits(((Float) list.get(i11)).floatValue()));
            i11++;
        }
    }

    public static void w(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        int i11 = 0;
        if (!(list instanceof w1)) {
            if (!z4) {
                while (i11 < list.size()) {
                    m1Var.j(i10, ((Integer) list.get(i11)).intValue());
                    i11++;
                }
                return;
            }
            m1Var.m(i10, 2);
            int i12 = 0;
            for (int i13 = 0; i13 < list.size(); i13++) {
                i12 += m1.c(((Integer) list.get(i13)).intValue());
            }
            m1Var.o(i12);
            while (i11 < list.size()) {
                m1Var.k(((Integer) list.get(i11)).intValue());
                i11++;
            }
            return;
        }
        w1 w1Var = (w1) list;
        if (!z4) {
            while (i11 < w1Var.c) {
                m1Var.j(i10, w1Var.n(i11));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < w1Var.c; i15++) {
            i14 += m1.c(w1Var.n(i15));
        }
        m1Var.o(i14);
        while (i11 < w1Var.c) {
            m1Var.k(w1Var.n(i11));
            i11++;
        }
    }

    public static void x(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof h2) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                m1Var.p(i10, ((Long) list.get(i11)).longValue());
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            i12 += m1.c(((Long) list.get(i13)).longValue());
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            m1Var.q(((Long) list.get(i11)).longValue());
            i11++;
        }
    }

    public static void y(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        int i11 = 0;
        if (!(list instanceof w1)) {
            if (!z4) {
                while (i11 < list.size()) {
                    m1Var.f(i10, ((Integer) list.get(i11)).intValue());
                    i11++;
                }
                return;
            }
            m1Var.m(i10, 2);
            int i12 = 0;
            for (int i13 = 0; i13 < list.size(); i13++) {
                ((Integer) list.get(i13)).getClass();
                i12 += 4;
            }
            m1Var.o(i12);
            while (i11 < list.size()) {
                m1Var.g(((Integer) list.get(i11)).intValue());
                i11++;
            }
            return;
        }
        w1 w1Var = (w1) list;
        if (!z4) {
            while (i11 < w1Var.c) {
                m1Var.f(i10, w1Var.n(i11));
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i14 = 0;
        for (int i15 = 0; i15 < w1Var.c; i15++) {
            w1Var.n(i15);
            i14 += 4;
        }
        m1Var.o(i14);
        while (i11 < w1Var.c) {
            m1Var.g(w1Var.n(i11));
            i11++;
        }
    }

    public static void z(int i10, List list, i2 i2Var, boolean z4) {
        if (list == null || list.isEmpty()) {
            return;
        }
        m1 m1Var = (m1) i2Var.a;
        if (list instanceof h2) {
            throw new ClassCastException();
        }
        int i11 = 0;
        if (!z4) {
            while (i11 < list.size()) {
                m1Var.h(i10, ((Long) list.get(i11)).longValue());
                i11++;
            }
            return;
        }
        m1Var.m(i10, 2);
        int i12 = 0;
        for (int i13 = 0; i13 < list.size(); i13++) {
            ((Long) list.get(i13)).getClass();
            i12 += 8;
        }
        m1Var.o(i12);
        while (i11 < list.size()) {
            m1Var.i(((Long) list.get(i11)).longValue());
            i11++;
        }
    }
}
