    ...
    */
    public j0(String str, char[] cArr) {
        ...
        for (int i10 = 0; i10 < cArr.length; i10++) {
            char c3 = cArr[i10];
            if (!(c3 < 128)) {
                throw new IllegalArgumentException(a.c("Non-ASCII character: %s", Character.valueOf(c3)));
            }
            if (!(bArr[c3] == -1)) {
                throw new IllegalArgumentException(a.c("Duplicate character: %s", Character.valueOf(c3)));
            }
            bArr[c3] = (byte) i10;
        }
    }
