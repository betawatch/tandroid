...
public class ActionBarMenuSubItem extends FrameLayout {
    ...
    protected void onMeasure(int i, int i2) {
        if (this.valueTextView.getVisibility() == 0) {
            ...
            this.valueTextView.measure(View.MeasureSpec.makeMeasureSpec(max, TLObject.FLAG_31), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(this.itemHeight), TLObject.FLAG_31));
            this.textView.setMaxWidth(Math.max(0, (max - this.valueTextView.getMeasuredWidth()) - AndroidUtilities.dp(8.0f)));
        } else {
            this.textView.setMaxWidth(ConnectionsManager.DEFAULT_DATACENTER_ID);
        }
        super.onMeasure(i, View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(this.itemHeight), TLObject.FLAG_30));
        if (!this.expandIfMultiline || this.textView.getLayout().getLineCount() <= 1) {
            return;
        }
        super.onMeasure(i, View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(this.itemHeight + 8), TLObject.FLAG_30));
    }
