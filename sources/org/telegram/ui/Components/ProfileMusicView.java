...
public class ProfileMusicView extends View {
    ...
    protected void onMeasure(int i, int i2) {
        super.onMeasure(View.MeasureSpec.makeMeasureSpec(View.MeasureSpec.getSize(i), TLObject.FLAG_30), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(37.0f), TLObject.FLAG_30));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float clamp01 = Utilities.clamp01(this.currentHeight / AndroidUtilities.dp(21.0f));
        ...
        float dp2 = AndroidUtilities.dp(16.0f) + dp;
        canvas.save();
        canvas.scale(scale, scale, getWidth() / 2.0f, getHeight() / 2.0f);
        this.rect.set((getWidth() - dp2) / 2.0f, AndroidUtilities.dp(10.0f), (getWidth() + dp2) / 2.0f, AndroidUtilities.dp(10.0f) + (AndroidUtilities.dp(17.0f) * clamp01));
        if (this.withShadows && SharedConfig.shadowsInSections) {
            this.backgroundPaint.setShadowLayer(AndroidUtilities.dpf2(2.0f), 0.0f, AndroidUtilities.dpf2(0.33f), Theme.multAlpha(167772160, clamp01));
            this.strokePaint.setShadowLayer(AndroidUtilities.dpf2(0.33f), 0.0f, 0.0f, Theme.multAlpha(201326592, clamp01));
            this.strokePaint.setColor(0);
        } else {
            this.backgroundPaint.setShadowLayer(0.0f, 0.0f, 0.0f, 0);
        }
        int alpha = this.backgroundPaint.getAlpha();
        this.backgroundPaint.setAlpha((int) (alpha * clamp01));
        if (this.withShadows && SharedConfig.shadowsInSections) {
        ...
        int i = (int) height;
        int i2 = dp3 / 2;
        this.icon.setBounds(0, i - i2, dp3, i + i2);
        this.icon.draw(canvas);
        canvas.translate(AndroidUtilities.dp(16.6f), 0.0f);
        this.author.draw(canvas, 0.0f, height, this.textColor, clamp01);
        canvas.translate(this.author.getWidth(), 0.0f);
        this.title.draw(canvas, 0.0f, height, this.textColor, clamp01 * 0.85f);
