...
public class PollContentDrawable extends Drawable implements DownloadController.FileDownloadProgressListener, SeekBar.SeekBarDelegate {
        ...
        if (this.isFile || this.isMusic) {
            ...
            if (this.isMusic) {
                ...
                if (floatValue > 0.0f) {
                    ...
                    this.seekBar.setSize(bounds.right - (AndroidUtilities.dp(56.0f) + dp), AndroidUtilities.dp(30.0f));
                    canvas.save();
                    float dp4 = AndroidUtilities.dp(45.0f) + dp;
                    this.seekBarX = dp4;
                    float dp5 = dp2 + dp3 + AndroidUtilities.dp(21.0f);
                    this.seekBarY = dp5;
    ...
    private void updatePlayingMessageProgress(boolean z) {
        ...
        boolean isCurrentPlayingMessageMusic = isCurrentPlayingMessageMusic();
        this.animatorIsPlaying.setValue(isCurrentPlayingMessageMusic, z);
        if (playingMessageObject == null || !isCurrentPlayingMessageMusic) {
            i = 0;
        } else {
