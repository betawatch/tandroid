...
public class FileState {
    ...
    public void checkState() {
        boolean z = false;
        boolean exists = this.attachPath != null ? new File(this.attachPath).exists() : false;
        if (!exists) {
            exists = FileLoader.getInstance(this.currentAccount).getPathToAttach(this.document).exists();
        }
