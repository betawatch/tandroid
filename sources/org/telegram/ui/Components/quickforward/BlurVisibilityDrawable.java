...
public class BlurVisibilityDrawable extends Drawable {
    ...
    public void render(int i, int i2, int i3, float f) {
        ...
        this.drawRunnable.draw(this.canvas, NotificationCenter.closeOtherAppActivities);
    ...
    public void draw(Canvas canvas) {
        int i = this.alpha;
        if (i == 255) {
            drawNormal(canvas, NotificationCenter.closeOtherAppActivities);
            return;
        }
        if (i == 0) {
            return;
        ...
        int clamp = MathUtils.clamp((int) (d2 * sqrt * 255.0d), 0, NotificationCenter.closeOtherAppActivities);
        drawBlur(canvas, MathUtils.clamp((int) (sqrt * 255.0d), 0, NotificationCenter.closeOtherAppActivities));
