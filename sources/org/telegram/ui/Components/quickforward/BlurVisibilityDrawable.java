...
public class BlurVisibilityDrawable extends Drawable {
    ...
    public void render(int i, int i2, int i3, float f) {
        ...
        this.drawRunnable.draw(this.canvas, NotificationCenter.needCheckSystemBarColors);
    ...
    public void draw(Canvas canvas) {
        int i = this.alpha;
        if (i == 255) {
            drawNormal(canvas, NotificationCenter.needCheckSystemBarColors);
            return;
        }
        if (i == 0) {
            return;
        ...
        int clamp = MathUtils.clamp((int) (d2 * sqrt * 255.0d), 0, NotificationCenter.needCheckSystemBarColors);
        drawBlur(canvas, MathUtils.clamp((int) (sqrt * 255.0d), 0, NotificationCenter.needCheckSystemBarColors));
