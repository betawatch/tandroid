...
public class QuickShareSelectorDrawable extends Drawable implements Animator.AnimatorListener {
    ...
    public static class Interpolators {
        ...
        static {
            ...
            closeAlpha = QuickShareSelectorDrawable.interpolator(new DecelerateInterpolator(), 0, NotificationCenter.invalidateMotionBackground, NotificationCenter.invalidateMotionBackground);
            closeAvatarPosition = QuickShareSelectorDrawable.interpolator(linearInterpolator, 0, NotificationCenter.invalidateMotionBackground, NotificationCenter.invalidateMotionBackground);
            closeAvatarAlpha = QuickShareSelectorDrawable.interpolator(new DecelerateInterpolator(), NotificationCenter.webViewResolved, NotificationCenter.invalidateMotionBackground, NotificationCenter.invalidateMotionBackground);
            buttonRotationUp = QuickShareSelectorDrawable.interpolator(new CubicBezierInterpolator(0.7f, -0.6f, 0.4f, 1.0f), 0, NotificationCenter.smsJobStatusUpdate, 560);
            buttonRotationDown = QuickShareSelectorDrawable.interpolator(new CubicBezierInterpolator(0.7f, -0.6f, 0.4f, 1.0f), NotificationCenter.smsJobStatusUpdate, 400, 560, true);
            buttonJumpUp = QuickShareSelectorDrawable.interpolator(new DecelerateInterpolator(), 0, 150, 560);
            buttonJumpDown = QuickShareSelectorDrawable.interpolator(new DecelerateInterpolator(), NotificationCenter.starOptionsLoaded, 425, 560);
            ...
            overshootCancel = QuickShareSelectorDrawable.interpolator(new DecelerateInterpolator(), NotificationCenter.smsJobStatusUpdate, 480, 560);
            ...
            avatarOvershootCancel = QuickShareSelectorDrawable.interpolator(new DecelerateInterpolator(), NotificationCenter.smsJobStatusUpdate, 460, 560);
        }
    }
    ...
    private void calculateCords() {
        ...
        this.bubbleOffset = (this.bubbleStart.right + ((float) AndroidUtilities.dp(48.0f))) + dp2 > ((float) this.parent.getMeasuredWidth()) ? Math.max(0.0f, (this.parent.getMeasuredWidth() - dp2) - this.bubbleStart.right) : ((this.bubbleStart.right + ((float) AndroidUtilities.dp(48.0f))) - ((float) getBubbleWidth())) - dp2 < 0.0f ? Math.max(0.0f, (dp2 + getBubbleWidth()) - this.bubbleStart.right) : AndroidUtilities.dp(48.0f);
    }
    ...
    public static /* synthetic */ float lambda$interpolator$1(boolean z, float f, float f2, Interpolator interpolator, float f3) {
        float f4 = (f3 - f) / (f2 - f);
        return z ? 1.0f - interpolator.getInterpolation(1.0f - MathUtils.clamp(f4, 0.0f, 1.0f)) : interpolator.getInterpolation(MathUtils.clamp(f4, 0.0f, 1.0f));
    }
    ...
    public void draw(Canvas canvas) {
        draw(canvas, NotificationCenter.suggestedLangpack, false);
    }
    ...
    public void draw(Canvas canvas, int i, boolean z) {
        ...
        if (blurVisibilityDrawable != null && !z) {
            ...
            if (this.selectedIndex != -1) {
                ...
                Bulletin.LottieLayout lottieLayout = this.bulletinLayout;
                this.avatarCells[this.selectedIndex].drawBlurredAvatar(canvas, fromTo(centerX, f, interpolation2), findParabola(centerX, centerY, f, f2, f3, (lottieLayout == null || !lottieLayout.top) ? Math.min(centerY, f2) - AndroidUtilities.dp(Sizes.CLOSE_AVATAR_JUMP_HEIGHT) : Math.max(centerY, f2) + AndroidUtilities.dp(Sizes.CLOSE_AVATAR_JUMP_HEIGHT), interpolation2), fromTo((AndroidUtilities.dp(Sizes.AVATAR) / 2.0f) + AndroidUtilities.dp(2.0f), AndroidUtilities.dp(12.0f), interpolation2), interpolation);
                return;
            }
            return;
        }
        ...
        if (!this.ballsAllowed || this.openAnimationCompleted) {
            float min = Math.min(this.bubbleCurrent.width(), this.bubbleCurrent.height()) / 2.0f;
            float min2 = Math.min(this.buttonCurrent.width(), this.buttonCurrent.height()) / 2.0f;
            canvas.drawRoundRect(this.bubbleCurrent, min, min, this.paintBubbleBg);
            if (!this.openAnimationCompleted) {
                canvas.drawRoundRect(this.buttonCurrent, min2, min2, this.paintBubbleBg);
            }
        } else {
