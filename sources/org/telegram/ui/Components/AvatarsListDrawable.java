...
public class AvatarsListDrawable extends Drawable {
    ...
    private final ListAnimator animator = new ListAnimator(new ListAnimator.Callback() { // from class: org.telegram.ui.Components.AvatarsListDrawable.1
    ...
    public void set(List list, boolean z) {
        ...
        while (it.hasNext()) {
            ...
            AvatarItem find = find(peerDialogId);
            if (find == null) {
                find = find(0L);
            }
            if (find == null) {
                find = new AvatarItem(this.parent);
                this.avatarItemsPool.add(find);
            }
            find.set(this.currentAccount, peerDialogId);
            arrayList.add(find);
            if (this.attached) {
                find.attach();
            }
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for (int size = this.animator.size() - 1; size >= 0; size--) {
            ...
            ((AvatarItem) entry.item).imageReceiver.setImageCoords(f4, f2, width, width);
