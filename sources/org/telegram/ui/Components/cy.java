package org.telegram.ui.Components;

import android.view.View;
import android.widget.TextView;
import androidx.recyclerview.widget.RecyclerView;
import java.util.ArrayList;
import java.util.regex.Pattern;
import org.telegram.messenger.MediaDataController;
import org.telegram.messenger.NotificationCenter;
import org.telegram.tgnet.TLRPC;

/* compiled from: r8-map-id-8159789691d0b3bb0641ef1f4646484974d69719884d0b33e845acec3d7c3062 */
/* loaded from: classes3.dex */
public final /* synthetic */ class cy implements View.OnClickListener {
    public final /* synthetic */ int a;
    public final /* synthetic */ dy b;

    public /* synthetic */ cy(dy dyVar, int i10) {
        this.a = i10;
        this.b = dyVar;
    }

    @Override // android.view.View.OnClickListener
    public final void onClick(View view) {
        TLRPC.StickerSet stickerSet;
        TLRPC.StickerSet stickerSet2;
        Integer num;
        View view2;
        int R;
        int i10;
        TLRPC.StickerSet stickerSet3;
        int i11 = this.a;
        dy dyVar = this.b;
        switch (i11) {
            case 0:
                yx yxVar = dyVar.s;
                if (yxVar != null && (stickerSet = yxVar.b) != null) {
                    lz lzVar = dyVar.E;
                    if (!lzVar.v2) {
                        lzVar.v2 = true;
                        ArrayList arrayList = new ArrayList(1);
                        TLRPC.TL_inputStickerSetID tL_inputStickerSetID = new TLRPC.TL_inputStickerSetID();
                        tL_inputStickerSetID.id = stickerSet.id;
                        tL_inputStickerSetID.access_hash = stickerSet.access_hash;
                        arrayList.add(tL_inputStickerSetID);
                        new ex(lzVar, lzVar.Y1, lzVar.getContext(), lzVar.Z1, arrayList, stickerSet).show();
                        break;
                    }
                }
                break;
            case 1:
                rg.p0 p0Var = dyVar.h;
                TextView textView = dyVar.f;
                TextView textView2 = dyVar.e;
                if (textView2 == null || textView2.getVisibility() != 0 || !textView2.isEnabled()) {
                    if (textView == null || textView.getVisibility() != 0 || !textView.isEnabled()) {
                        if (p0Var != null && p0Var.getVisibility() == 0 && p0Var.r.isEnabled()) {
                            p0Var.performClick();
                            break;
                        }
                    } else {
                        textView.performClick();
                        break;
                    }
                } else {
                    textView2.performClick();
                    break;
                }
                break;
            case 2:
                lz lzVar2 = dyVar.E;
                ArrayList arrayList2 = lzVar2.p1;
                yx yxVar2 = dyVar.s;
                if (yxVar2 != null && (stickerSet2 = yxVar2.b) != null) {
                    yxVar2.f = true;
                    ux uxVar = lzVar2.R;
                    int i12 = lzVar2.c1;
                    ArrayList arrayList3 = lzVar2.q1;
                    xx xxVar = lzVar2.P;
                    if (!arrayList2.contains(Long.valueOf(stickerSet2.id))) {
                        arrayList2.add(Long.valueOf(dyVar.s.b.id));
                    }
                    dyVar.a(true);
                    int i13 = 0;
                    while (true) {
                        if (i13 >= xxVar.getChildCount()) {
                            num = null;
                            view2 = null;
                        } else if (!(xxVar.getChildAt(i13) instanceof ay) || (R = RecyclerView.R((view2 = xxVar.getChildAt(i13)))) < 0 || (i10 = uxVar.w.get(R)) < 0 || i10 >= arrayList3.size() || arrayList3.get(i10) == null || dyVar.s == null || ((yx) arrayList3.get(i10)).b.id != dyVar.s.b.id) {
                            i13++;
                        } else {
                            num = Integer.valueOf(R);
                        }
                    }
                    if (num != null) {
                        uxVar.E(num.intValue(), view2);
                    }
                    if (dyVar.n == null) {
                        TLRPC.TL_inputStickerSetID tL_inputStickerSetID2 = new TLRPC.TL_inputStickerSetID();
                        TLRPC.StickerSet stickerSet4 = dyVar.s.b;
                        tL_inputStickerSetID2.id = stickerSet4.id;
                        tL_inputStickerSetID2.access_hash = stickerSet4.access_hash;
                        TLRPC.TL_messages_stickerSet stickerSet5 = MediaDataController.getInstance(i12).getStickerSet((TLRPC.InputStickerSet) tL_inputStickerSetID2, true);
                        if (stickerSet5 != null && stickerSet5.set != null) {
                            org.telegram.ui.ActionBar.m2 m2Var = lzVar2.Y1;
                            if (m2Var == null) {
                                m2Var = new ai.y3(dyVar, 6);
                            }
                            tv.W(m2Var, stickerSet5, true, null, new yp(dyVar, 14));
                            break;
                        } else {
                            NotificationCenter.getInstance(i12).addObserver(dyVar, NotificationCenter.groupStickersDidLoad);
                            MediaDataController mediaDataController = MediaDataController.getInstance(i12);
                            dyVar.n = tL_inputStickerSetID2;
                            mediaDataController.getStickerSet((TLRPC.InputStickerSet) tL_inputStickerSetID2, false);
                            break;
                        }
                    }
                }
                break;
            case 3:
                lz lzVar3 = dyVar.E;
                yx yxVar3 = dyVar.s;
                if (yxVar3 != null && (stickerSet3 = yxVar3.b) != null) {
                    yxVar3.f = false;
                    ArrayList arrayList4 = lzVar3.p1;
                    int i14 = lzVar3.c1;
                    arrayList4.remove(Long.valueOf(stickerSet3.id));
                    dyVar.a(true);
                    px pxVar = lzVar3.I;
                    if (pxVar != null) {
                        pxVar.p(lzVar3.getEmojipacks());
                    }
                    lzVar3.U(lzVar3.Q.I0());
                    if (dyVar.r == null) {
                        TLRPC.TL_inputStickerSetID tL_inputStickerSetID3 = new TLRPC.TL_inputStickerSetID();
                        TLRPC.StickerSet stickerSet6 = dyVar.s.b;
                        tL_inputStickerSetID3.id = stickerSet6.id;
                        tL_inputStickerSetID3.access_hash = stickerSet6.access_hash;
                        TLRPC.TL_messages_stickerSet stickerSet7 = MediaDataController.getInstance(i14).getStickerSet((TLRPC.InputStickerSet) tL_inputStickerSetID3, true);
                        if (stickerSet7 != null && stickerSet7.set != null) {
                            org.telegram.ui.ActionBar.m2 m2Var2 = lzVar3.Y1;
                            if (m2Var2 == null) {
                                m2Var2 = new ai.y3(dyVar, 6);
                            }
                            org.telegram.ui.ActionBar.m2 m2Var3 = m2Var2;
                            vw vwVar = new vw(2, dyVar, stickerSet7);
                            Pattern pattern = tv.V;
                            if (m2Var3.getFragmentView() != null) {
                                MediaDataController.getInstance(m2Var3.getCurrentAccount()).toggleStickerSet(m2Var3.getFragmentView().getContext(), stickerSet7, 0, m2Var3, true, true, vwVar, false);
                                break;
                            }
                        } else {
                            NotificationCenter.getInstance(i14).addObserver(dyVar, NotificationCenter.groupStickersDidLoad);
                            MediaDataController mediaDataController2 = MediaDataController.getInstance(i14);
                            dyVar.r = tL_inputStickerSetID3;
                            mediaDataController2.getStickerSet((TLRPC.InputStickerSet) tL_inputStickerSetID3, false);
                            break;
                        }
                    }
                }
                break;
            case 4:
                my myVar = dyVar.E.t1;
                if (myVar != null) {
                    myVar.q();
                    break;
                }
                break;
            case 5:
                my myVar2 = dyVar.E.t1;
                if (myVar2 != null) {
                    myVar2.q();
                    break;
                }
                break;
            default:
                my myVar3 = dyVar.E.t1;
                if (myVar3 != null) {
                    myVar3.q();
                    break;
                }
                break;
        }
    }
}
