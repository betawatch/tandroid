package org.telegram.ui.Components;

import androidx.recyclerview.widget.RecyclerView;
import org.telegram.messenger.MediaDataController;
import org.telegram.tgnet.TLRPC;

/* compiled from: r8-map-id-11b2057e7e9050c40bb40722946bba2b5eb90c231d630684b08af6cb92d5aac3 */
/* loaded from: classes3.dex */
public final class vw0 extends f2.g0 {
    public int e;
    public final /* synthetic */ cx0 f;

    public vw0(cx0 cx0Var) {
        this.f = cx0Var;
        this.d = 15;
        this.e = -1;
    }

    @Override // f2.d0
    public final boolean n(RecyclerView recyclerView, f2.q1 q1Var, f2.q1 q1Var2) {
        int i9 = q1Var.f;
        if (i9 == 3 || i9 != q1Var2.f) {
            return false;
        }
        cx0 cx0Var = this.f;
        if (cx0Var.O == null) {
            return false;
        }
        int b10 = q1Var.b();
        int b11 = q1Var2.b();
        cx0Var.O.documents.add(b11, cx0Var.O.documents.remove(b10));
        cx0Var.d.p(b10, b11);
        this.e = b11;
        return true;
    }

    @Override // f2.d0
    public final void p(f2.q1 q1Var, int i9) {
        cx0 cx0Var = this.f;
        if (i9 != 0 || cx0Var.f == null || this.e <= 0) {
            if (i9 == 2) {
                cx0Var.f = ((org.telegram.ui.Cells.d8) q1Var.a).getSticker();
            }
        } else {
            TLRPC.TL_stickers_changeStickerPosition tL_stickers_changeStickerPosition = new TLRPC.TL_stickers_changeStickerPosition();
            tL_stickers_changeStickerPosition.position = this.e;
            tL_stickers_changeStickerPosition.sticker = MediaDataController.getInputStickerSetItem(cx0Var.f, "").document;
            this.e = -1;
            cx0Var.f = null;
        }
    }

    @Override // f2.d0
    public final void q(f2.q1 q1Var) {
    }

    @Override // f2.d0
    public final void o(RecyclerView recyclerView, f2.q1 q1Var, f2.q1 q1Var2, int i9, int i10, int i11) {
    }
}
