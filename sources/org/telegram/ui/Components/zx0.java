...
public final class zx0 extends Drawable {
    ...
    public final void draw(Canvas canvas) {
        ...
        canvas.save();
        if (e9 < 0.5f) {
            float abs = Math.abs(e9 - 0.5f) + 0.5f;
            canvas.scale(abs, abs, centerX, centerY);
        }
        canvas.save();
        if (e9 > 0.5f) {
            float abs2 = Math.abs(e9 - 0.5f) + 0.5f;
            float f10 = -abs2;
