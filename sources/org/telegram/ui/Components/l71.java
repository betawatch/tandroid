...
public final class l71 extends Drawable {
    ...
    */
    public final void draw(Canvas canvas) {
        ...
        int B = this.d ? org.telegram.messenger.q.B(16.0f, bounds.width() / 4, width) : org.telegram.messenger.q.D(16.0f, bounds.width() / 4, width);
        canvas.save();
        if (this.e) {
            if (this.f == null) {
                this.f = new Path();
        ...
        canvas.drawCircle(((Math.max(bounds.width(), bounds.height()) / 4) * (this.d ? -1 : 1)) + B, AndroidUtilities.dp(16.0f) + height, Math.max(bounds.width(), bounds.height()) / 2, paint);
        canvas.restore();
        String str = this.p;
        if (str != null) {
            canvas.drawText(str, (AndroidUtilities.dp(32.0f) * (this.d ? -1 : 1)) + B, AndroidUtilities.dp(15.0f) + AndroidUtilities.dp(32.0f) + height, textPaint);
        }
        canvas.save();
        float f11 = this.q;
        float f12 = B;
