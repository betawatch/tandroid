...
public class CapsuleBlobDrawable extends Drawable {
    ...
    private void rebuildGeometry() {
        ...
        float maxOutwardExcursion = maxOutwardExcursion() + AndroidUtilities.dp(1.0f);
        float min = (Math.min(r0.width(), r0.height()) / 2.0f) - AndroidUtilities.dp(2.0f);
        if (maxOutwardExcursion > min) {
            maxOutwardExcursion = Math.max(0.0f, min);
        }
        this.halfWidth = (r0.width() / 2.0f) - maxOutwardExcursion;
        float height = (r0.height() / 2.0f) - maxOutwardExcursion;
        this.halfHeight = height;
        float f = this.halfWidth;
        if (f < 1.0f || height < 1.0f) {
            return;
        }
        float min2 = Math.min(this.cornerRadius, Math.min(f, height));
        this.radius = min2;
        float f2 = this.halfWidth - min2;
        this.innerX = f2;
        float f3 = this.halfHeight - min2;
        this.innerY = f3;
        float f4 = f2 * 2.0f;
        this.straightH = f4;
        float f5 = f3 * 2.0f;
        this.straightV = f5;
        float f6 = this.curvatureBoost * 1.5707964f * min2;
        this.weightedArc = f6;
        float f7 = (f4 * 2.0f) + (f5 * 2.0f);
        this.weightedTotal = (f6 * 4.0f) + f7;
        float f8 = f7 + (min2 * 6.2831855f);
        this.perimeter = f8;
        int max = Math.max(12, Math.min(80, Math.round(f8 / this.targetSpacing)));
        if (max != this.big.count()) {
            this.big.resize(max);
            this.small.resize(max);
        }
    }
    ...
    public void draw(Canvas canvas) {
        ...
        long min = (this.running || this.colorProgress < 1.0f) ? Math.min(40L, Math.max(0L, elapsedRealtime - this.lastFrameTime)) : 0L;
        this.lastFrameTime = elapsedRealtime;
        if (LiteMode.isEnabled(512) && min > 0) {
            updateAmplitude(min);
            this.breathPhase += (min / this.breathPeriodMs) * 6.2831855f;
            this.big.update(this.amplitude);
            this.small.update(this.amplitude);
        }
        float f = this.colorProgress;
        if (f < 1.0f && min > 0) {
            float f2 = f + (min / 250.0f);
    ...
    private void drawLayer(Canvas canvas, Layer layer, float f, float f2, float f3) {
        ...
        if (this.neighborCoherence > 0.0f) {
            ...
            while (i2 < 2) {
                ...
                while (i3 < count) {
                    int i4 = i3 + 1;
                    float f10 = (fArr[i3 == 0 ? count - 1 : i3 - 1] + fArr[i4 == count ? 0 : i4]) * 0.5f;
                    float f11 = fArr[i3];
        ...
        for (int i5 = 0; i5 < count; i5++) {
            pointAt((i5 / count) + fArr3[i5], this.sample);
        ...
        while (i6 < count) {
            int i7 = i6 + 1;
            int i8 = i7 < count ? i7 : 0;
            float[] fArr6 = layer.px;
            float f15 = fArr6[i8] - fArr6[i6];
            float[] fArr7 = layer.py;
            float f16 = fArr7[i8] - fArr7[i6];
            float sqrt = ((float) Math.sqrt((f15 * f15) + (f16 * f16))) / 3.0f;
            Path path = this.path;
            float[] fArr8 = layer.px;
            float f17 = fArr8[i6];
            float[] fArr9 = layer.tx;
            float f18 = (fArr9[i6] * sqrt) + f17;
            float[] fArr10 = layer.py;
            float f19 = fArr10[i6];
            float[] fArr11 = layer.ty;
            float f20 = f19 + (fArr11[i6] * sqrt);
            float f21 = fArr8[i8];
            float f22 = f21 - (fArr9[i8] * sqrt);
            float f23 = fArr10[i8];
            path.cubicTo(f18, f20, f22, f23 - (fArr11[i8] * sqrt), f21, f23);
            i6 = i7;
        }
        this.path.close();
    ...
    private void pointAt(float f, float[] fArr) {
        float floor = (f - ((float) Math.floor(f))) * this.weightedTotal;
        float f2 = this.straightH;
        if (floor < f2) {
            fArr[0] = (-this.innerX) + floor;
    ...
    private static final class Layer {
        ...
        void applyColor() {
            ...
            this.paint.setAlpha((this.baseAlpha * this.mAlpha) / NotificationCenter.didReceiveSmsCode);
        }
