...
public final class h71 {
    ...
    public final void a(Canvas canvas, View view) {
        ...
        float f14 = this.b;
        float min = Math.min(this.c, f14);
        this.c = min;
        float lerp2 = AndroidUtilities.lerp(min, f14, 0.5f);
        this.c = lerp2;
        float abs = Math.abs(f14 - lerp2);
        View view3 = this.v;
        if (abs > 0.005f) {
            view3.invalidate();
        }
        ...
        if (f16 != 1.0f) {
            ...
            if (f17 >= 1.0f) {
                ...
                view.invalidate();
                float interpolation = rr.f.getInterpolation(this.C);
                f15 = (f15 * interpolation) + ((1.0f - interpolation) * this.B);
            }
        }
        ...
        if (this.q) {
            ...
            if (f20 > 0.0f) {
                f7 = 2.0f;
                rectF.right = AndroidUtilities.lerp((f20 * (this.h - r14)) + (S / 2.0f), view3.getWidth() - (this.z * 2.0f), this.y) + this.z;
            ...
            float f21 = this.s;
            if (f21 > 0.0f) {
                rectF.right = AndroidUtilities.lerp((f21 * (this.h - r13)) + (S / f7), view3.getWidth() - (this.z * f7), this.y) + this.z;
            ...
            float f22 = this.p;
            float f23 = this.r;
            float f24 = (this.s * f23) + ((1.0f - f23) * f22);
            if (f24 > 0.0f) {
                rectF.right = AndroidUtilities.lerp((f24 * (this.h - r13)) + (S / 2.0f), view3.getWidth() - (this.z * 2.0f), this.y) + this.z;
                g(1.0f - this.y, this.l);
                b(canvas, rectF, Q);
            }
        }
        ...
        if (this.t != dp) {
            ...
            if (f25 < dp) {
                c10 = 0;
                float e = a4.a.e(j3, 60.0f, AndroidUtilities.dp(1.0f), f25);
                this.t = e;
                if (e > dp) {
                    this.t = dp;
                }
                ...
                c10 = 0;
                float c12 = org.telegram.messenger.ul.c(j3, 60.0f, AndroidUtilities.dp(1.0f), f25);
                this.t = c12;
                if (c12 < dp) {
                    this.t = dp;
                }
        ...
        if (d > 0.0f) {
            ...
            rectF.right = lerp4;
            rectF.left = AndroidUtilities.lerp(f26, lerp4, 1.0f - d);
            ...
            canvas.drawCircle(AndroidUtilities.lerp((S / f7) + this.E, (this.E / (this.h - S)) * (view2.getWidth() - (this.z * f7)), this.y) + this.z, rectF.centerY(), lerp3 * d, Q);
        } else {
            f10 = 0.2f;
            f11 = 8.0f;
        }
        ...
        rectF.right = AndroidUtilities.lerp(f28 + f15, c() * (view2.getWidth() - (this.z * f7)), this.y) + f27;
        if (this.y > 0.0f && rectF.width() > 0.0f) {
            R.setAlpha((int) (this.y * 255.0f * f10));
            b(canvas, rectF, R);
        }
        g(1.0f, i0.a.d(this.y, this.n, this.A));
        b(canvas, rectF, Q);
        ...
        float f29 = ((this.f || this.g) ? this.d : this.c) / (this.h - S);
        int size = this.F.size() - 1;
        while (true) {
            if (size < 0) {
                size = -1;
                break;
            } else if (((Float) ((Pair) this.F.get(size)).first).floatValue() - 0.001f <= f29) {
                break;
            } else {
                size--;
            }
        ...
        float f30 = (this.h - (S / f7)) + this.z;
        float abs2 = Math.abs(lerp5 - f30) - AndroidUtilities.dp(16.0f);
        float f31 = this.P;
        if (f31 <= 0.0f || Math.abs(f31 - abs2) <= 0.01f) {
            f12 = 3.0f;
            f13 = 16.0f;
        } else {
        ...
        float interpolation2 = rr.f.getInterpolation(this.M);
        canvas.save();
        int i15 = this.i;
        canvas.translate(((lerp6 - f30) * this.y) + lerp5, AndroidUtilities.lerp((i15 + i10) / f7, i15 - AndroidUtilities.dp(f12), this.y) + AndroidUtilities.dp(12.0f));
        if (this.K[c11] != null) {
            canvas.save();
            if (this.N != 0) {
                canvas.translate((AndroidUtilities.dp(f13) * (-this.N) * interpolation2) + AndroidUtilities.dp(f11), 0.0f);
            }
        ...
        if (this.K[c10] != null) {
            canvas.save();
            if (this.N != 0) {
                canvas.translate(com.google.android.gms.internal.vision.e2.z(1.0f, interpolation2, AndroidUtilities.dp(f13) * this.N, AndroidUtilities.dp(f11)), 0.0f);
            }
            canvas.translate(0.0f, (-this.K[c10].getHeight()) / f7);
            this.L.setAlpha((int) (org.telegram.messenger.z0.A(1.0f, this.y, 255.0f, interpolation2) * this.I));
    ...
    */
    public final void b(Canvas canvas, RectF rectF, Paint paint) {
        ...
        char c11;
        h71 h71Var = this;
        float dp = AndroidUtilities.dp(AndroidUtilities.lerp(2, 1, h71Var.y));
        ...
        float lerp = AndroidUtilities.lerp(S / 2.0f, 0.0f, h71Var.y) + h71Var.z;
        float lerp2 = AndroidUtilities.lerp(h71Var.h - (S / 2.0f), h71Var.v.getWidth() - (h71Var.z * 2.0f), h71Var.y) + h71Var.z;
        AndroidUtilities.rectTmp.set(rectF);
        float dp2 = AndroidUtilities.dp(h71Var.I * 1.0f) / 2.0f;
        if (U == null) {
            U = new Path();
        }
        U.reset();
        ...
        while (true) {
            i10 = -1;
            if (i11 >= h71Var.F.size()) {
                i11 = -1;
                break;
            } else if (((Float) ((Pair) h71Var.F.get(i11)).first).floatValue() >= dp3) {
                break;
            } else {
                i11++;
            }
        ...
        int size = h71Var.F.size() - 1;
        while (true) {
            if (size < 0) {
                break;
            }
            if (1.0f - ((Float) ((Pair) h71Var.F.get(size)).first).floatValue() >= dp3) {
                i10 = size + 1;
                break;
            }
            size--;
        ...
        while (i13 <= i12) {
            if (i13 == i11) {
                ...
                c10 = 1;
                floatValue = ((Float) ((Pair) h71Var.F.get(i13 - 1)).first).floatValue();
            }
            float floatValue2 = i13 == i12 ? 1.0f : ((Float) ((Pair) h71Var.F.get(i13)).first).floatValue();
            while (i13 != i12 && i13 != 0 && i13 < h71Var.F.size() - 1 && ((Float) ((Pair) h71Var.F.get(i13)).first).floatValue() - floatValue <= dp3) {
                i13++;
                floatValue2 = ((Float) ((Pair) h71Var.F.get(i13)).first).floatValue();
            }
    ...
    public final boolean e(float f7, float f10, int i10) {
        ...
        if (i10 == 0) {
            ...
            if (this.f) {
                ...
                float f11 = i18;
                this.c = f11;
                if (i10 == 1 && (g71Var = this.j) != null) {
                    g71Var.b(f11 / (this.h - S));
                }
                this.f = false;
                AndroidUtilities.runOnUIThread(new w61(this, 2), 50L);
                return true;
            }
    ...
    public final void h(float f7, boolean z10) {
        ...
        if (z10) {
            if (Math.abs(ceil - this.b) > AndroidUtilities.dp(10.0f)) {
                float interpolation = rr.f.getInterpolation(this.C);
                this.B = (int) com.google.android.gms.internal.vision.e2.z(1.0f, interpolation, this.B, this.b * interpolation);
                this.C = 0.0f;
            } else if (this.C == 1.0f) {
                this.C = 0.0f;
                this.B = this.b;
