...
public class MotionBackgroundPaint {
    ...
    public MotionBackgroundPaint() {
        ...
        this.patterAlphaBitmapMemo = new BitmapMemoizedMetadata(new BitmapMemoizedMetadata.Provider() { // from class: org.telegram.ui.Components.MotionBackgroundPaint$$ExternalSyntheticLambda3
    ...
    private Paint getPaintHwAgsl(Bitmap bitmap, Bitmap bitmap2, int i, int i2, int i3) {
        ...
        if (i3 >= 0) {
            ...
            float clamp = MathUtils.clamp((i2 * (-i3)) / 25500.0f, 0.0f, 1.0f);
            if (upVar || this.lastHwIntensity != clamp || this.lastModeHwAgsl != 2) {
                this.lastModeHwAgsl = 2;
                this.lastHwIntensity = clamp;
