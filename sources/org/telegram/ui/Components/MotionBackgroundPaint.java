...
public class MotionBackgroundPaint {
    ...
    private final BitmapMemoizedMetadata patterAlphaBitmapMemo = new BitmapMemoizedMetadata(new BitmapMemoizedMetadata.Provider() { // from class: org.telegram.ui.Components.MotionBackgroundPaint$$ExternalSyntheticLambda0
        @Override // org.telegram.ui.Components.blur3.utils.BitmapMemoizedMetadata.Provider
        public final Object get(Bitmap bitmap) {
            Bitmap alphaChannel;
            alphaChannel = MotionBackgroundPaint.getAlphaChannel(bitmap);
            return alphaChannel;
        }
    });
    ...
    private static class AgslImpl {
        ...
        Paint getPaint(Bitmap bitmap, Bitmap bitmap2, Bitmap bitmap3, int i, int i2) {
            ...
            if (i2 >= 0) {
                ...
                float clamp = MathUtils.clamp((i * (-i2)) / 25500.0f, 0.0f, 1.0f);
                if (upVar || this.lastIntensity != clamp || this.lastMode != 2) {
                    this.lastMode = 2;
                    this.lastIntensity = clamp;
                    ...
                    this.runtimeShaderNegative.shader.setFloatUniform("intensity", clamp);
