...
public class DialogsActivityTopPanelLayout extends AnimatedLinearLayout {
    ...
    private void checkBoundsAndClipping() {
        ...
        float min = Math.min(AndroidUtilities.dp(this.defaultRadiusDp), Math.min(this.clipRectF.width(), this.clipRectF.height()) / 2.0f);
    ...
    protected boolean drawChild(Canvas canvas, View view, long j) {
        boolean isCallView = isCallView(view);
        if (isCallView && this.exceptCall) {
            return false;
        }
        if (isCallView || !this.onlyCall) {
