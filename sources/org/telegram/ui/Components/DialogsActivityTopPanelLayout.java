...
public class DialogsActivityTopPanelLayout extends AnimatedLinearLayout {
    ...
    private void checkBoundsAndClipping() {
        ...
        float min = Math.min(AndroidUtilities.dp(24.0f), Math.min(this.clipRectF.width(), this.clipRectF.height()) / 2.0f);
        this.clipPath.rewind();
        this.clipPath.addRoundRect(this.clipRectF, min, min, Path.Direction.CW);
    ...
    public void updateColors() {
        ...
        this.fadeDrawable.setColors(new int[]{ColorUtils.setAlphaComponent(color, (alpha * NotificationCenter.starGiftSoldOut) / NotificationCenter.cameraInitied), ColorUtils.setAlphaComponent(color, (alpha * NotificationCenter.didUpdateTonGiftStickers) / NotificationCenter.cameraInitied), ColorUtils.setAlphaComponent(color, (alpha * NotificationCenter.fileNewChunkAvailable) / NotificationCenter.cameraInitied), ColorUtils.setAlphaComponent(color, 0)});
        invalidate();
    }
