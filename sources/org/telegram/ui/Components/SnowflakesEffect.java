...
public class SnowflakesEffect {
    ...
    public SnowflakesEffect(int i) {
        this.viewType = i;
        int i2 = i == 0 ? 100 : NotificationCenter.dialogsUnreadPollVotesCounterChanged;
        this.maxCount = i2;
    ...
    public void updateColors() {
        int i = this.forcedColor;
        if (i == 0) {
            i = Theme.getColor(this.colorKey) & (-1644826);
        }
        if (this.color != i) {
            this.color = i;
            this.particlePaint.setColor(i);
            this.particleThinPaint.setColor(i);
        }
    }
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (this.batchParticlesBuffer != null) {
            int min = Math.min(this.maxCount, this.particles.size());
    ...
    public static Bitmap createParticlesBitmap(boolean z) {
        ...
        paint.setColor(-1);
        Bitmap createBitmap = Bitmap.createBitmap(z ? AndroidUtilities.dp(20.0f) : AndroidUtilities.dp(10.0f), AndroidUtilities.dp(10.0f), Bitmap.Config.ARGB_8888);
        ...
        while (i < 6) {
            ...
            float f5 = f2;
            float f6 = dp2;
            canvas.drawLine(dp, dp2, dp + cos, f4, paint);
            double d2 = (float) (d - 1.5707963267948966d);
            double d3 = f;
            double d4 = dpf22;
            float f7 = dp + (cos * 0.66f);
            float f8 = f6 + f3;
            canvas.drawLine(f7, f8, dp + ((float) ((Math.cos(d2) * d3) - (Math.sin(d2) * d4))), f6 + ((float) ((Math.sin(d2) * d3) + (Math.cos(d2) * d4))), paint);
            canvas.drawLine(f7, f8, dp + ((float) (((-Math.cos(d2)) * d3) - (Math.sin(d2) * d4))), f6 + ((float) (((-Math.sin(d2)) * d3) + (Math.cos(d2) * d4))), paint);
            f2 = f5 + 1.0471976f;
            i++;
            createBitmap = bitmap;
