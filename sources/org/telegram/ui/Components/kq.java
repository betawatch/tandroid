...
public class kq extends Path {
    ...
    public final void addRect(RectF rectF, Path.Direction direction) {
        if (Build.VERSION.SDK_INT < 34 || !this.c) {
            ...
            float f12 = rectF.top;
            int i10 = this.f;
            super.addRect(f11, f12 - i10, rectF.right + i9, rectF.bottom + i10, direction);
            return;
        }
        ArrayList arrayList = this.a;
        if (arrayList.size() <= 0 || !((RectF) j3.r0.j(1, arrayList)).contains(rectF)) {
            if (arrayList.size() <= 0 || Math.abs(rectF.top - ((RectF) j3.r0.j(1, arrayList)).top) > this.d || Math.abs(rectF.bottom - ((RectF) j3.r0.j(1, arrayList)).bottom) > this.d) {
                ArrayList arrayList2 = g;
                RectF rectF2 = (arrayList2 == null || arrayList2.size() <= 0) ? new RectF() : (RectF) g.remove(0);
                rectF2.set(rectF);
                arrayList.add(rectF2);
            } else {
                ((RectF) j3.r0.j(1, arrayList)).union(rectF);
            }
            this.b = false;
        }
    }
    ...
    public void addRect(float f10, float f11, float f12, float f13, Path.Direction direction) {
        ...
        if (Build.VERSION.SDK_INT >= 34 && this.c) {
            ArrayList arrayList = this.a;
            if (arrayList.size() <= 0 || !((RectF) j3.r0.j(1, arrayList)).contains(f10, f11, f12, f13)) {
                if (arrayList.size() > 0 && Math.abs(f11 - ((RectF) j3.r0.j(1, arrayList)).top) <= this.d && Math.abs(f13 - ((RectF) j3.r0.j(1, arrayList)).bottom) <= this.d) {
                    ((RectF) j3.r0.j(1, arrayList)).union(f10, f11, f12, f13);
                } else {
                    ArrayList arrayList2 = g;
        ...
        float f14 = f10 - i9;
        int i10 = this.f;
        super.addRect(f14, f11 - i10, f12 + i9, f13 + i10, direction);
    }
}
