...
public abstract class NinePatchBuilder {
    ...
    */
    public static NinePatchDrawable createNinePatch(Bitmap[] bitmapArr, float[] fArr, float f, float f2, float f3, int i, NinePathRenderer ninePathRenderer) {
        ...
        int ceil2 = ceil + ((int) Math.ceil(Math.max(0.0f, -f2)));
        int ceil3 = ceil + ((int) Math.ceil(Math.max(0.0f, f2)));
        int ceil4 = ceil + ((int) Math.ceil(Math.max(0.0f, -f3)));
        ...
        if (!z2 || (bitmap = bitmapArr[0]) == null || bitmap.isRecycled() || !bitmap.isMutable()) {
            z = z2;
        } else {
            z = z2;
            if (bitmap.getWidth() == i2 && bitmap.getHeight() == i3) {
                ...
                if (bitmap.getConfig() == Bitmap.Config.ARGB_8888) {
                    ...
                    ninePathRenderer.draw(new Canvas(bitmap2), new RectF(ceil2, ceil4, ceil6, ceil7), new float[]{f4, max2, max3, max4, max5, max6, max7, max8});
                    float max11 = Math.max(f4, max7);
                    float max12 = Math.max(max3, max5);
                    float max13 = Math.max(max2, max4);
                    float max14 = Math.max(max8, max6);
                    int clamp = MathUtils.clamp(ceil2 + ((int) Math.ceil(max11)), 1, i2 - 2);
                    int clamp2 = MathUtils.clamp((i2 - ceil3) - ((int) Math.ceil(max12)), clamp + 1, i2 - 1);
                    int clamp3 = MathUtils.clamp(((int) Math.ceil(max13)) + ceil4, 1, i3 - 2);
                    return new NinePatchDrawable(ApplicationLoader.applicationContext.getResources(), bitmap2, createNinePatchChunk(clamp, clamp2, clamp3, MathUtils.clamp((i3 - ceil5) - ((int) Math.ceil(max14)), clamp3 + 1, i3 - 1), ceil2, ceil4, ceil3, ceil5, i).array(), new Rect(ceil2, ceil4, ceil3, ceil5), null);
                }
                bitmap = null;
                if (bitmap == null) {
                }
                ...
                ninePathRenderer.draw(new Canvas(bitmap22), new RectF(ceil2, ceil4, ceil6, ceil7), new float[]{f4, max2, max3, max4, max5, max6, max7, max8});
                float max112 = Math.max(f4, max7);
                float max122 = Math.max(max3, max5);
                float max132 = Math.max(max2, max4);
                float max142 = Math.max(max8, max6);
                int clamp4 = MathUtils.clamp(ceil2 + ((int) Math.ceil(max112)), 1, i2 - 2);
                int clamp22 = MathUtils.clamp((i2 - ceil3) - ((int) Math.ceil(max122)), clamp4 + 1, i2 - 1);
                int clamp32 = MathUtils.clamp(((int) Math.ceil(max132)) + ceil4, 1, i3 - 2);
                return new NinePatchDrawable(ApplicationLoader.applicationContext.getResources(), bitmap22, createNinePatchChunk(clamp4, clamp22, clamp32, MathUtils.clamp((i3 - ceil5) - ((int) Math.ceil(max142)), clamp32 + 1, i3 - 1), ceil2, ceil4, ceil3, ceil5, i).array(), new Rect(ceil2, ceil4, ceil3, ceil5), null);
            }
        }
        f4 = max;
        bitmap = null;
        ...
        ninePathRenderer.draw(new Canvas(bitmap222), new RectF(ceil2, ceil4, ceil6, ceil7), new float[]{f4, max2, max3, max4, max5, max6, max7, max8});
        float max1122 = Math.max(f4, max7);
        float max1222 = Math.max(max3, max5);
        float max1322 = Math.max(max2, max4);
        float max1422 = Math.max(max8, max6);
        int clamp42 = MathUtils.clamp(ceil2 + ((int) Math.ceil(max1122)), 1, i2 - 2);
        int clamp222 = MathUtils.clamp((i2 - ceil3) - ((int) Math.ceil(max1222)), clamp42 + 1, i2 - 1);
        int clamp322 = MathUtils.clamp(((int) Math.ceil(max1322)) + ceil4, 1, i3 - 2);
        return new NinePatchDrawable(ApplicationLoader.applicationContext.getResources(), bitmap222, createNinePatchChunk(clamp42, clamp222, clamp322, MathUtils.clamp((i3 - ceil5) - ((int) Math.ceil(max1422)), clamp322 + 1, i3 - 1), ceil2, ceil4, ceil3, ceil5, i).array(), new Rect(ceil2, ceil4, ceil3, ceil5), null);
    }
