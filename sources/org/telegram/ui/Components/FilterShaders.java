...
public class FilterShaders {
    ...
    public static String vertexShaderForOptimizedBlurOfRadius(int i, float f) {
        ...
        while (true) {
            ...
            fArr[i3] = sqrt;
            f2 = i3 == 0 ? f2 + sqrt : (float) (f2 + (sqrt * 2.0d));
            i3++;
        }
    ...
    private static class ToneCurve {
            ...
            while (i < length - 1) {
                ...
                while (true) {
                    ...
                    if (i3 < ((int) f)) {
                        ...
                        double d2 = (f2 - r12) / d;
                        double d3 = 1.0d - d2;
                        double[] dArr = secondDerivative;
                        int i4 = length;
                        float f3 = (float) ((pointF.y * d3) + (pointF2.y * d2) + (((d * d) / 6.0d) * (((((d3 * d3) * d3) - d3) * dArr[i]) + ((((d2 * d2) * d2) - d2) * dArr[i2]))));
                        if (f3 > 255.0f) {
                            f3 = 255.0f;
                        } else if (f3 < 0.0f) {
                            f3 = 0.0f;
        ...
        private double[] secondDerivative(ArrayList arrayList) {
            ...
            while (true) {
                ...
                float f3 = pointF3.y;
                float f4 = pointF2.y;
                dArr2[i2] = ((f3 - f4) / d6) - ((f4 - pointF.y) / d4);
                i2 = i3;
                d = d3;
                d2 = d5;
            }
    ...
    private boolean setupExternalShaders() {
        ...
        if (hDRType == 1) {
            ...
            readRes = hDRType == 2 ? AndroidUtilities.readRes(R.raw.hdr2sdr_pq) : "";
        }
        ...
        while (true) {
            ...
            if (i2 < (z2 ? 2 : 1)) {
                if (i2 == 1 && z2) {
                    ...
                    if (loadShader5 == 0 || loadShader6 == 0) {
                        loadShader5 = loadShader(35633, "attribute vec4 position;uniform mat4 videoMatrix;attribute vec4 inputTexCoord;varying vec2 vTextureCoord;void main() {gl_Position = position;vTextureCoord = vec2(videoMatrix * inputTexCoord).xy;}");
                        loadShader6 = loadShader(35632, String.format(Locale.US, "%1$s\nprecision highp float;varying vec2 vTextureCoord;uniform %2$s sTexture;vec3 rgb_to_hsv(vec3 c) {vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);vec4 p = c.g < c.b ? vec4(c.bg, K.wz) : vec4(c.gb, K.xy);vec4 q = c.r < p.x ? vec4(p.xyw, c.r) : vec4(c.r, p.yzx);float d = q.x - min(q.w, q.y);float e = 1.0e-10;return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);}void main() {vec4 texel = texture2D(sTexture, vTextureCoord);gl_FragColor = vec4(rgb_to_hsv(texel.rgb), texel.a);}", str3, str));
                    }
                } else {
                    str = str4;
                ...
                GLES20.glAttachShader(this.rgbToHsvShaderProgram[i2], loadShader5);
    ...
    public void drawCustomParamsPass() {
        ...
        if (filterShadersDelegate == null || filterShadersDelegate.shouldShowOriginal()) {
            ...
            GLES20.glUniform3f(this.highlightsTintColorHandle, ((tintHighlightsColor >> 16) & NotificationCenter.didReceiveSmsCode) / 255.0f, ((tintHighlightsColor >> 8) & NotificationCenter.didReceiveSmsCode) / 255.0f, (tintHighlightsColor & NotificationCenter.didReceiveSmsCode) / 255.0f);
            GLES20.glUniform1f(this.highlightsTintIntensityHandle, this.delegate.getTintHighlightsIntensityValue());
            GLES20.glUniform3f(this.shadowsTintColorHandle, ((tintShadowsColor >> 16) & NotificationCenter.didReceiveSmsCode) / 255.0f, ((tintShadowsColor >> 8) & NotificationCenter.didReceiveSmsCode) / 255.0f, (tintShadowsColor & NotificationCenter.didReceiveSmsCode) / 255.0f);
            ...
            if (shouldDrawCurvesPass) {
                ...
                GLES20.glActiveTexture(33985);
                GLES20.glBindTexture(3553, this.curveTextures[0]);
                GLES20.glTexImage2D(3553, 0, 6408, NotificationCenter.dialogPhotosUpdate, 1, 0, 6408, 5121, fillAndGetCurveBuffer);
        ...
        if (bitmap2 != null && !bitmap2.isRecycled()) {
            ...
            if ((z && (this.renderBufferWidth > photoSize || this.renderBufferHeight > photoSize)) || i % 360 != 0) {
                if (!z || (this.renderBufferWidth <= photoSize && this.renderBufferHeight <= photoSize)) {
                    ...
                    f = photoSize / bitmap2.getWidth();
                    float height = photoSize / bitmap2.getHeight();
                    if (f < height) {
                        this.renderBufferWidth = (int) photoSize;
                        this.renderBufferHeight = (int) (bitmap2.getHeight() * f);
                    } else {
                        this.renderBufferHeight = (int) photoSize;
                        this.renderBufferWidth = (int) (bitmap2.getWidth() * height);
                        f = height;
                    }
                }
                int i4 = i % 360;
