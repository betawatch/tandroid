...
public class FilterShaders {
            ...
            while (i < length - 1) {
                ...
                while (true) {
                    ...
                    if (i3 < ((int) f)) {
                        ...
                        double d2 = (f2 - r12) / d;
                        double d3 = 1.0d - d2;
                        float f3 = (float) ((pointF.y * d3) + (pointF2.y * d2) + (((d * d) / 6.0d) * (((((d3 * d3) * d3) - d3) * secondDerivative[i]) + ((((d2 * d2) * d2) - d2) * secondDerivative[i2]))));
                        if (f3 > 255.0f) {
                            f3 = 255.0f;
                        } else if (f3 < 0.0f) {
                            f3 = 0.0f;
    ...
    public void drawCustomParamsPass() {
        ...
        if (filterShadersDelegate == null || filterShadersDelegate.shouldShowOriginal()) {
            ...
            GLES20.glUniform3f(this.highlightsTintColorHandle, ((tintHighlightsColor >> 16) & NotificationCenter.didReplacedPhotoInMemCache) / 255.0f, ((tintHighlightsColor >> 8) & NotificationCenter.didReplacedPhotoInMemCache) / 255.0f, (tintHighlightsColor & NotificationCenter.didReplacedPhotoInMemCache) / 255.0f);
            GLES20.glUniform1f(this.highlightsTintIntensityHandle, this.delegate.getTintHighlightsIntensityValue());
            GLES20.glUniform3f(this.shadowsTintColorHandle, ((tintShadowsColor >> 16) & NotificationCenter.didReplacedPhotoInMemCache) / 255.0f, ((tintShadowsColor >> 8) & NotificationCenter.didReplacedPhotoInMemCache) / 255.0f, (tintShadowsColor & NotificationCenter.didReplacedPhotoInMemCache) / 255.0f);
            ...
            if (shouldDrawCurvesPass) {
                ...
                GLES20.glActiveTexture(33985);
                GLES20.glBindTexture(3553, this.curveTextures[0]);
                GLES20.glTexImage2D(3553, 0, 6408, NotificationCenter.channelRecommendationsLoaded, 1, 0, 6408, 5121, fillAndGetCurveBuffer);
