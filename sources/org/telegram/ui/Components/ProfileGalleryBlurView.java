    ...
    public void doBlur() {
        ...
        for (int i = 0; i < 6; i += 2) {
            ...
            if (blurBitmapHolder2 != null && !blurBitmapHolder2.destroying && blurBitmapHolder2.hasContent) {
                ...
                Utilities.stackBlurBitmap(bitmap, Math.max(10, bitmap.getWidth() / NotificationCenter.suggestedFiltersLoaded));
                synchronized (this.lock) {
                    blurBitmapHolder3.clear();
        ...
        synchronized (this.lock) {
            try {
                ...
                if (this.paints[0].getShader() != null) {
                    ...
                    this.paints[0].setAlpha(NotificationCenter.needCheckSystemBarColors);
                    canvas.restore();
                }
                if (this.offset != 0 && this.paints[1].getShader() != null) {
                    ...
                    this.paints[1].setAlpha(NotificationCenter.needCheckSystemBarColors);
                    canvas.restore();
                }
            } catch (Throwable th) {
                throw th;
