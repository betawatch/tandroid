...
public final class a61 extends Drawable {
    ...
    public a61(boolean z10) {
        ...
        for (int i9 = 0; i9 < 3; i9++) {
            ...
            if (i9 == 0) {
                int i10 = i9 * 2;
                this.c.moveTo(AndroidUtilities.dp(iArr[i10]), AndroidUtilities.dp(iArr[i10 + 1]));
            } else {
                int i11 = i9 * 2;
                this.c.lineTo(AndroidUtilities.dp(iArr[i11]), AndroidUtilities.dp(iArr[i11 + 1]));
            }
        }
        this.c.close();
    }
    ...
    */
    public final void draw(Canvas canvas) {
        ...
        int A = this.d ? org.telegram.messenger.l0.A(16.0f, bounds.width() / 4, width) : org.telegram.messenger.l0.C(16.0f, bounds.width() / 4, width);
        canvas.save();
        if (this.e) {
            if (this.f == null) {
                this.f = new Path();
            }
            int i9 = bounds.left + (bounds.top << 8) + (bounds.bottom << 16) + (bounds.right << 24);
            if (this.g != i9) {
                this.f.reset();
        ...
        canvas.drawCircle(((Math.max(bounds.width(), bounds.height()) / 4) * (this.d ? -1 : 1)) + A, AndroidUtilities.dp(16.0f) + height, Math.max(bounds.width(), bounds.height()) / 2, paint);
        canvas.restore();
        String str = this.p;
        if (str != null) {
            canvas.drawText(str, (AndroidUtilities.dp(32.0f) * (this.d ? -1 : 1)) + A, AndroidUtilities.dp(15.0f) + AndroidUtilities.dp(32.0f) + height, textPaint);
        }
        canvas.save();
        float f12 = this.q;
        float f13 = A;
