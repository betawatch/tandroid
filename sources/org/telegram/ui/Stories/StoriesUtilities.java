...
public abstract class StoriesUtilities {
    ...
    */
    public static void drawAvatarWithStory(long j, Canvas canvas, ImageReceiver imageReceiver, boolean z, AvatarStoryParams avatarStoryParams) {
        ...
        float f9 = storiesController.hasUnreadStoriesLive(avatarStoryParams.dialogId) == 2 ? avatarStoryParams.progressToSegments : 0.0f;
        ...
        if ((avatarStoryParams.prevState == 1 && avatarStoryParams.progressToSate != f2) || avatarStoryParams.currentState == 1) {
            if (i4 == 2) {
            ...
            boolean z5 = avatarStoryParams.prevState == 1 && avatarStoryParams.progressToSate != f2;
            ...
            if (z5) {
                ...
                gradientTools.paint.setAlpha((int) (avatarStoryParams.alpha * 255.0f * (f2 - f11)));
            } else {
                gradientTools.paint.setAlpha((int) (avatarStoryParams.alpha * 255.0f * f11));
                dp3 = f12 + (AndroidUtilities.dp(5.0f) * (f2 - f11));
            }
        ...
        if ((i9 != 2 || avatarStoryParams.progressToSate == f2) && avatarStoryParams.currentState != 2) {
            i5 = 3;
            f4 = 1.0f;
        } else {
            boolean z6 = i9 == 2 && avatarStoryParams.progressToSate != f2;
            ...
            if (z6) {
                ...
                paint9.setAlpha((int) (avatarStoryParams.alpha * 255.0f * (f2 - f11)));
            } else {
                paint9.setAlpha((int) (avatarStoryParams.alpha * 255.0f * f11));
                dp2 = f8 + (AndroidUtilities.dp(5.0f) * (f2 - f11));
            }
            ...
            if (avatarStoryParams.drawSegments) {
                i5 = 3;
                f4 = 1.0f;
                drawSegmentsInternal(canvas, storiesController, imageReceiver, avatarStoryParams, paint9, paint6, paint7, paint8, z4);
            } else {
                i5 = 3;
                f4 = 1.0f;
                drawCircleInternal(canvas, imageReceiver.getParentView(), avatarStoryParams, paint9, z4);
            }
        }
        if ((avatarStoryParams.prevState == i5 && avatarStoryParams.progressToSate != f4) || avatarStoryParams.currentState == i5) {
            ...
            if (avatarStoryParams.prevState == i5 && avatarStoryParams.progressToSate != f4) {
                dp = f7 + (AndroidUtilities.dp(7.0f) * f11);
                paint10.setAlpha((int) (avatarStoryParams.alpha * 255.0f * (f4 - f11)));
            } else {
            ...
            rectF5.inset(dp, dp);
            boolean z7 = avatarStoryParams.drawSegments;
            if (z7 && avatarStoryParams.currentState == i5) {
                float f13 = avatarStoryParams.progressToProgressSegments;
                if (f13 != f4) {
                    float f14 = f13 + f3;
                    avatarStoryParams.progressToProgressSegments = f14;
                    if (f14 > f4) {
                        avatarStoryParams.progressToProgressSegments = f4;
                    }
                    float f15 = avatarStoryParams.progressToSegments;
                    avatarStoryParams.progressToSegments = f4 - avatarStoryParams.progressToProgressSegments;
                    drawSegmentsInternal(canvas, storiesController, imageReceiver, avatarStoryParams, paint10, paint2, paint3, paint4, z4);
            ...
            if (z7) {
                ...
                if (unreadState2 == 2) {
                    ...
                    if (unreadState2 != i5) {
                        ...
                        avatarStoryParams.drawnLive = z2;
                        if (f9 > 0.0f) {
                            drawLive(canvas, avatarStoryParams.originalAvatarRect, f5, imageReceiver.getVisible(), false);
                        }
                        f6 = avatarStoryParams.progressToSate;
                        if (f6 != f4) {
                            float f16 = f6 + (AndroidUtilities.screenRefreshTime / 250.0f);
                            avatarStoryParams.progressToSate = f16;
                            if (f16 > f4) {
                                avatarStoryParams.progressToSate = f4;
                            }
                            if (imageReceiver.getParentView() != null) {
                                imageReceiver.invalidate();
                                imageReceiver.getParentView().invalidate();
