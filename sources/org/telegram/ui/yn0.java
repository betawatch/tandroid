...
public final class yn0 implements TextWatcher {
    ...
    public final void afterTextChanged(Editable editable) {
        ...
        if (wo0Var.C0.invoice.max_tip_amount != 0) {
            ...
            long j3 = wo0Var.C0.invoice.max_tip_amount;
            if (longValue4 > j3) {
                wo0Var.H0 = Long.valueOf(j3);
            }
        }
        int selectionStart = wo0Var.f[0].getSelectionStart();
        wo0Var.m0 = true;
        if (wo0Var.H0.longValue() == 0) {
            wo0Var.f[0].setText("");
        } else {
            EditTextBoldCursor editTextBoldCursor = wo0Var.f[0];
            str2 = LocaleController.getInstance().formatCurrencyString(wo0Var.H0.longValue(), false, z10, true, wo0Var.C0.invoice.currency);
            editTextBoldCursor.setText(str2);
        }
        if (longValue < wo0Var.H0.longValue() && longValue != 0 && this.a && selectionStart >= 0) {
            EditTextBoldCursor editTextBoldCursor2 = wo0Var.f[0];
            editTextBoldCursor2.setSelection(Math.min(selectionStart, editTextBoldCursor2.length()));
        } else if (this.c && this.d != wo0Var.f[0].length()) {
            EditTextBoldCursor editTextBoldCursor3 = wo0Var.f[0];
            editTextBoldCursor3.setSelection(Math.max(0, Math.min(selectionStart, editTextBoldCursor3.length())));
        } else if (this.f || !z10 || a2 < 0) {
            EditTextBoldCursor editTextBoldCursor4 = wo0Var.f[0];
            editTextBoldCursor4.setSelection(editTextBoldCursor4.length());
        } else {
            int a10 = a(str2);
            if (a10 > 0) {
                wo0Var.f[0].setSelection(a10 + 1);
            } else {
                EditTextBoldCursor editTextBoldCursor5 = wo0Var.f[0];
