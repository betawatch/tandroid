        ...
        while (true) {
            ...
            if (tLObject instanceof TLRPC.ChannelParticipant) {
                if (MessageObject.getPeerId(((TLRPC.ChannelParticipant) tLObject).peer) == j3) {
                    ...
                    tL_channelParticipantAdmin.inviter_id = vrVar.getUserConfig().getClientUserId();
                    if (j3 > 0) {
                ...
                int indexOf = vrVar.s.participants.participants.indexOf(chatParticipant);
                if (indexOf >= 0) {
                    vrVar.s.participants.participants.set(indexOf, tL_chatParticipantAdmin);
                }
                vrVar.r0();
            }
            i12++;
