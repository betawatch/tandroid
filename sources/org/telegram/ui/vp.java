package org.telegram.ui;

import android.animation.AnimatorSet;
import android.animation.ValueAnimator;
import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.graphics.Path;
import android.graphics.PorterDuff;
import android.graphics.PorterDuffColorFilter;
import android.graphics.PorterDuffXfermode;
import android.graphics.RectF;
import android.text.StaticLayout;
import android.text.TextPaint;
import android.view.View;
import java.util.ArrayList;
import org.telegram.messenger.AccountInstance;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.DialogObject;
import org.telegram.messenger.ImageLocation;
import org.telegram.messenger.ImageReceiver;
import org.telegram.messenger.MessagesController;
import org.telegram.messenger.NotificationCenter;
import org.telegram.messenger.UserConfig;
import org.telegram.tgnet.TLRPC;

/* compiled from: r8-map-id-33f3ee7b3837766f245c82aac5a618a539713405f9dc265162d35c247069ed49 */
/* loaded from: classes3.dex */
public final class vp implements NotificationCenter.NotificationCenterDelegate {
    public int B;
    public final ImageReceiver C;
    public TLRPC.Chat D;
    public TLRPC.TL_forumTopic E;
    public long F;
    public AnimatorSet G;
    public float H;
    public float I;
    public boolean J;
    public boolean K;
    public float L;
    public long M;
    public float N;
    public boolean O;
    public float P;
    public final View Q;
    public long R;
    public boolean S;
    public boolean T;
    public final boolean U;
    public bj V;
    public long W;
    public View X;
    public boolean Y;
    public final org.telegram.ui.Components.sq Z;
    public int a;
    public final int[] a0;
    public int b;
    public final int b0;
    public int c;
    public final int c0;
    public float d;
    public final int d0;
    public final Paint e;
    public final long e0;
    public final TextPaint f;
    public final long f0;
    public final org.telegram.ui.ActionBar.f6 g0;
    public final TextPaint h;
    public org.telegram.ui.Components.l5 h0;
    public final Paint n;
    public final Path r;
    public StaticLayout s;
    public StaticLayout v;
    public StaticLayout w;
    public int x;
    public int y;

    public vp(int i10, View view, long j10, int i11, int i12, long j11, org.telegram.ui.ActionBar.f6 f6Var) {
        Paint paint = new Paint(1);
        this.e = paint;
        TextPaint textPaint = new TextPaint(1);
        this.f = textPaint;
        TextPaint textPaint2 = new TextPaint(1);
        this.h = textPaint2;
        Paint paint2 = new Paint(1);
        this.n = paint2;
        this.r = new Path();
        this.F = 0L;
        this.Y = true;
        org.telegram.ui.Components.sq sqVar = new org.telegram.ui.Components.sq(null, true, null);
        this.Z = sqVar;
        this.a0 = new int[3];
        this.Q = view;
        this.b0 = i10;
        this.f0 = j10;
        this.c0 = i11;
        this.d0 = i12;
        this.e0 = j11;
        this.U = MessagesController.getInstance(i10).isForum(j10);
        this.g0 = f6Var;
        this.C = new ImageReceiver(view);
        paint.setStrokeWidth(AndroidUtilities.dpf2(2.8f));
        paint.setStrokeCap(Paint.Cap.ROUND);
        sqVar.z = 3;
        sqVar.I = 1;
        sqVar.g = true;
        sqVar.d = d("paintChatActionBackground");
        sqVar.e = textPaint;
        textPaint.setTextSize(AndroidUtilities.dp(13.0f));
        textPaint.setTypeface(AndroidUtilities.bold());
        textPaint2.setTextSize(AndroidUtilities.dp(14.0f));
        paint2.setColor(-16777216);
        paint2.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
    }

    public static TLRPC.Dialog c(long j10, int i10, int i11, boolean z4, int[] iArr) {
        ArrayList<TLRPC.Dialog> dialogs;
        TLRPC.Dialog c3;
        long j11;
        int i12;
        int[] iArr2;
        MessagesController messagesController = AccountInstance.getInstance(UserConfig.selectedAccount).getMessagesController();
        if (iArr != null) {
            iArr[0] = 0;
            iArr[1] = i10;
            iArr[2] = i11;
        }
        if (i11 != 0) {
            MessagesController.DialogFilter dialogFilter = messagesController.dialogFiltersById.get(i11);
            if (dialogFilter == null) {
                return null;
            }
            dialogs = dialogFilter.dialogs;
        } else {
            dialogs = messagesController.getDialogs(i10);
        }
        if (dialogs == null) {
            return null;
        }
        for (int i13 = 0; i13 < dialogs.size(); i13++) {
            TLRPC.Dialog dialog = dialogs.get(i13);
            TLRPC.Chat chat = messagesController.getChat(Long.valueOf(-dialog.id));
            if (chat != null && dialog.id != j10 && dialog.unread_count > 0 && DialogObject.isChannel(dialog) && !chat.megagroup && !messagesController.isPromoDialog(dialog.id, false) && messagesController.getRestrictionReason(chat.restriction_reason) == null) {
                return dialog;
            }
        }
        if (!z4) {
            return null;
        }
        if (i11 != 0) {
            int i14 = 0;
            while (i14 < messagesController.dialogFilters.size()) {
                int i15 = messagesController.dialogFilters.get(i14).id;
                if (i11 != i15) {
                    long j12 = j10;
                    int i16 = i10;
                    int[] iArr3 = iArr;
                    TLRPC.Dialog c10 = c(j12, i16, i15, false, iArr3);
                    j11 = j12;
                    i12 = i16;
                    iArr2 = iArr3;
                    if (c10 != null) {
                        if (iArr2 != null) {
                            iArr2[0] = 1;
                        }
                        return c10;
                    }
                } else {
                    j11 = j10;
                    i12 = i10;
                    iArr2 = iArr;
                }
                i14++;
                i10 = i12;
                j10 = j11;
                iArr = iArr2;
            }
        }
        long j13 = j10;
        int i17 = i10;
        int[] iArr4 = iArr;
        for (int i18 = 0; i18 < messagesController.dialogsByFolder.size(); i18++) {
            int keyAt = messagesController.dialogsByFolder.keyAt(i18);
            if (i17 != keyAt && (c3 = c(j13, keyAt, 0, false, iArr4)) != null) {
                if (iArr4 != null) {
                    iArr4[0] = 1;
                }
                return c3;
            }
        }
        return null;
    }

    public final void a(Canvas canvas, sj sjVar, float f10, float f11) {
        int i10;
        int i11;
        RectF rectF;
        int i12;
        float f12;
        float f13;
        float f14;
        org.telegram.ui.Components.sq sqVar;
        float f15;
        float f16;
        TextPaint textPaint;
        float f17;
        int i13;
        float f18;
        float f19;
        int i14;
        int i15;
        Paint paint;
        TextPaint textPaint2;
        ImageReceiver imageReceiver;
        RectF rectF2;
        float f20;
        float f21;
        float w10;
        Canvas canvas2 = canvas;
        if (this.X != sjVar) {
            this.X = sjVar;
            org.telegram.ui.Components.l5 l5Var = this.h0;
            if (l5Var != null) {
                l5Var.a(sjVar);
            }
        }
        org.telegram.ui.Components.sq sqVar2 = this.Z;
        sqVar2.H = sjVar;
        RectF rectF3 = sqVar2.f;
        float dp = AndroidUtilities.dp(110.0f) * f10;
        if (dp < AndroidUtilities.dp(8.0f)) {
            return;
        }
        float f22 = f10 < 0.2f ? 5.0f * f10 * f11 : f11;
        org.telegram.ui.ActionBar.j6.q(0.0f, sjVar.getMeasuredHeight() - dp, this.c, sjVar.getMeasuredHeight());
        int i16 = org.telegram.ui.ActionBar.j6.ic;
        org.telegram.ui.ActionBar.f6 f6Var = this.g0;
        int v02 = org.telegram.ui.ActionBar.j6.v0(i16, f6Var);
        TextPaint textPaint3 = this.f;
        textPaint3.setColor(v02);
        int v03 = org.telegram.ui.ActionBar.j6.v0(i16, f6Var);
        Paint paint2 = this.e;
        paint2.setColor(v03);
        this.h.setColor(org.telegram.ui.ActionBar.j6.v0(org.telegram.ui.ActionBar.j6.Vd, f6Var));
        int alpha = d("paintChatActionBackground").getAlpha();
        int alpha2 = org.telegram.ui.ActionBar.j6.h2.getAlpha();
        int alpha3 = textPaint3.getAlpha();
        int alpha4 = paint2.getAlpha();
        org.telegram.ui.ActionBar.j6.h2.setAlpha((int) (alpha2 * f22));
        int i17 = (int) (alpha * f22);
        d("paintChatActionBackground").setAlpha(i17);
        int i18 = (int) (alpha3 * f22);
        textPaint3.setAlpha(i18);
        if ((f10 < 1.0f || this.N >= 1.0f) && (f10 >= 1.0f || this.N != 1.0f)) {
            i10 = i18;
            i11 = i17;
            rectF = rectF3;
            i12 = alpha2;
        } else {
            i10 = i18;
            i11 = i17;
            long currentTimeMillis = System.currentTimeMillis();
            rectF = rectF3;
            i12 = alpha2;
            if (currentTimeMillis - this.M > 100) {
                try {
                    sjVar.performHapticFeedback(3, 2);
                } catch (Exception unused) {
                }
                this.M = currentTimeMillis;
            }
            this.N = f10;
        }
        if (f10 == 1.0f && !this.J) {
            this.J = true;
            this.K = true;
            g(sjVar, true);
            this.R = System.currentTimeMillis();
        } else if (f10 != 1.0f && this.J) {
            this.J = false;
            g(sjVar, false);
        }
        float f23 = this.c / 2.0f;
        float f24 = this.I * (-AndroidUtilities.dp(4.0f));
        float f25 = this.O ? dp - f24 : dp;
        float f26 = f25 / 2.0f;
        float max = Math.max(0.0f, Math.min(this.d, (f26 - (AndroidUtilities.dp(16.0f) * f10)) - AndroidUtilities.dp(4.0f)));
        float max2 = (1.0f - this.H) * ((Math.max(0.0f, Math.min(this.d * f10, f26 - (AndroidUtilities.dp(8.0f) * f10))) * 2.0f) - AndroidUtilities.dp2(16.0f));
        float dp2 = AndroidUtilities.dp(56.0f);
        float f27 = this.H;
        float f28 = (dp2 * f27) + max2;
        if (f27 < 1.0f || this.O) {
            f12 = 36.0f;
            float f29 = -f25;
            float f30 = f25;
            float dp3 = (this.H * (AndroidUtilities.dp(56.0f) + f29)) + ((1.0f - this.H) * (-AndroidUtilities.dp(8.0f)));
            RectF rectF4 = AndroidUtilities.rectTmp;
            int i19 = i11;
            rectF4.set(f23 - max, f29, f23 + max, dp3);
            if (this.H <= 0.0f || this.O) {
                f13 = 1.0f;
            } else {
                float dp4 = AndroidUtilities.dp(16.0f) * this.H;
                rectF4.inset(dp4, dp4);
                f13 = 1.0f - this.H;
            }
            if (this.T) {
                Path path = this.r;
                path.reset();
                float width = rectF4.width() * 0.2f;
                float width2 = rectF4.width() * 0.1f;
                float width3 = rectF4.width() * 0.03f;
                f14 = f29;
                float f31 = width2 / 2.0f;
                float height = rectF4.height() - width2;
                sqVar = sqVar2;
                f15 = f28;
                path.moveTo(rectF4.right, rectF4.top + width + width2);
                float f32 = -width;
                path.rQuadTo(0.0f, f32, f32, f32);
                float f33 = width * 2.0f;
                float f34 = f31 * 2.0f;
                f16 = f22;
                path.rLineTo((((-(rectF4.width() - f33)) / 2.0f) + f34) - width3, 0.0f);
                float f35 = -f31;
                float f36 = f35 / 2.0f;
                float f37 = f35 * 2.0f;
                textPaint = textPaint3;
                float f38 = (-width2) / 2.0f;
                path.rQuadTo(f36, 0.0f, f37, f38);
                path.rQuadTo(f36, f38, f37, f38);
                path.rLineTo(((-(rectF4.width() - f33)) / 2.0f) + f34 + width3, 0.0f);
                path.rQuadTo(f32, 0.0f, f32, width);
                path.rLineTo(0.0f, (height + width2) - f33);
                path.rQuadTo(0.0f, width, width, width);
                path.rLineTo(rectF4.width() - f33, 0.0f);
                path.rQuadTo(width, 0.0f, width, f32);
                path.rLineTo(0.0f, -(height - f33));
                path.close();
                canvas2.drawPath(path, d("paintChatActionBackground"));
                if (f6Var != null ? f6Var.m0() : org.telegram.ui.ActionBar.j6.a1()) {
                    canvas2.drawPath(path, org.telegram.ui.ActionBar.j6.h2);
                }
            } else {
                f14 = f29;
                sqVar = sqVar2;
                f15 = f28;
                f16 = f22;
                textPaint = textPaint3;
                int alpha5 = d("paintChatActionBackground").getAlpha();
                d("paintChatActionBackground").setAlpha((int) (alpha5 * f13));
                float f39 = this.d;
                canvas2.drawRoundRect(rectF4, f39, f39, d("paintChatActionBackground"));
                d("paintChatActionBackground").setAlpha(alpha5);
                if (f6Var != null ? f6Var.m0() : org.telegram.ui.ActionBar.j6.a1()) {
                    int alpha6 = org.telegram.ui.ActionBar.j6.h2.getAlpha();
                    org.telegram.ui.ActionBar.j6.h2.setAlpha((int) (alpha6 * f13));
                    float f40 = this.d;
                    canvas2.drawRoundRect(rectF4, f40, f40, org.telegram.ui.ActionBar.j6.h2);
                    org.telegram.ui.ActionBar.j6.h2.setAlpha(alpha6);
                }
            }
            float w11 = e2.c.w(1.0f, f10, AndroidUtilities.dp(8.0f), f14 + AndroidUtilities.dp(24.0f)) - (AndroidUtilities.dp(36.0f) * this.H);
            canvas2.save();
            rectF4.inset(AndroidUtilities.dp(1.0f), AndroidUtilities.dp(1.0f));
            canvas2.clipRect(rectF4);
            float f41 = this.H;
            if (f41 > 0.0f) {
                paint2.setAlpha((int) ((1.0f - f41) * 255.0f));
            }
            canvas2.save();
            float dp5 = (AndroidUtilities.dp(24.0f) * f10) / AndroidUtilities.dpf2(24.0f);
            canvas2.scale(dp5, dp5, f23, w11 - AndroidUtilities.dp(20.0f));
            canvas2.translate(f23 - AndroidUtilities.dp2(12.0f), w11 - AndroidUtilities.dp(12.0f));
            f17 = f23;
            i13 = i10;
            f18 = f24;
            f19 = f30;
            i14 = i19;
            i15 = alpha;
            canvas2.drawLine(AndroidUtilities.dpf2(12.5f), AndroidUtilities.dpf2(4.0f), AndroidUtilities.dpf2(12.5f), AndroidUtilities.dpf2(22.0f), paint2);
            canvas.drawLine(AndroidUtilities.dpf2(3.5f), AndroidUtilities.dpf2(12.0f), AndroidUtilities.dpf2(12.5f), AndroidUtilities.dpf2(3.5f), paint2);
            canvas2 = canvas;
            canvas2.drawLine(AndroidUtilities.dpf2(21.5f), AndroidUtilities.dpf2(12.0f), AndroidUtilities.dpf2(12.5f), AndroidUtilities.dpf2(3.5f), paint2);
            canvas2.restore();
            if (this.O) {
                float A = e2.c.A(f14 - AndroidUtilities.dp(2.0f), this.H, (1.0f - this.H) * (((-AndroidUtilities.dp(8.0f)) - (AndroidUtilities.dp2(8.0f) * f10)) - f15), f18);
                paint2.setAlpha(alpha4);
                canvas2.save();
                canvas2.scale(f10, f10, f17, AndroidUtilities.dp(28.0f) + A);
                float dp6 = A + AndroidUtilities.dp(28.0f);
                if (this.K) {
                    float f42 = this.L;
                    if (f42 < 1.0f) {
                        float f43 = f42 + 0.07272727f;
                        this.L = f43;
                        if (f43 > 1.0f) {
                            this.L = 1.0f;
                        }
                    }
                    float f44 = this.L;
                    float f45 = f44 > 0.5f ? 1.0f : f44 / 0.5f;
                    float f46 = f44 < 0.5f ? 0.0f : (f44 - 0.5f) / 0.5f;
                    canvas2.save();
                    canvas2.clipRect(rectF4);
                    canvas2.translate(f17 - AndroidUtilities.dp(24.0f), dp6 - AndroidUtilities.dp(24.0f));
                    float dp7 = AndroidUtilities.dp(16.0f);
                    float dp8 = AndroidUtilities.dp(26.0f);
                    float dp9 = AndroidUtilities.dp(22.0f);
                    float dp10 = AndroidUtilities.dp(32.0f);
                    float dp11 = AndroidUtilities.dp(32.0f);
                    float dp12 = AndroidUtilities.dp(20.0f);
                    float f47 = 1.0f - f45;
                    canvas.drawLine(dp7, dp8, (dp9 * f45) + (dp7 * f47), (f45 * dp10) + (f47 * dp8), paint2);
                    if (f46 > 0.0f) {
                        float f48 = 1.0f - f46;
                        float f49 = (dp11 * f46) + (dp9 * f48);
                        float f50 = (dp12 * f46) + (f48 * dp10);
                        canvas2 = canvas;
                        canvas2.drawLine(dp9, dp10, f49, f50, paint2);
                    } else {
                        canvas2 = canvas;
                    }
                    paint = paint2;
                    canvas2.restore();
                } else {
                    paint = paint2;
                }
                canvas2.restore();
            } else {
                paint = paint2;
            }
            canvas2.restore();
        } else {
            paint = paint2;
            sqVar = sqVar2;
            f15 = f28;
            f16 = f22;
            textPaint = textPaint3;
            i13 = i10;
            f18 = f24;
            f12 = 36.0f;
            f17 = f23;
            i15 = alpha;
            i14 = i11;
            f19 = f25;
        }
        if (this.s == null || this.H <= 0.0f) {
            textPaint2 = textPaint;
        } else {
            d("paintChatActionBackground").setAlpha(i14);
            textPaint2 = textPaint;
            textPaint2.setAlpha(i13);
            float dp13 = (((1.0f - this.H) * AndroidUtilities.dp(20.0f)) - (AndroidUtilities.dp(f12) * this.H)) + f18;
            RectF rectF5 = AndroidUtilities.rectTmp;
            float f51 = (r4 - this.x) / 2.0f;
            rectF5.set(f51, dp13, this.c - f51, this.s.getHeight() + dp13);
            rectF5.inset(-AndroidUtilities.dp(8.0f), -AndroidUtilities.dp(4.0f));
            canvas2.drawRoundRect(rectF5, AndroidUtilities.dp(15.0f), AndroidUtilities.dp(15.0f), d("paintChatActionBackground"));
            if (f6Var != null ? f6Var.m0() : org.telegram.ui.ActionBar.j6.a1()) {
                canvas2.drawRoundRect(rectF5, AndroidUtilities.dp(15.0f), AndroidUtilities.dp(15.0f), org.telegram.ui.ActionBar.j6.h2);
            }
            canvas2.save();
            canvas2.translate((this.c - this.x) / 2.0f, dp13);
            this.s.draw(canvas2);
            canvas2.restore();
        }
        if (!this.O && f15 > 0.0f) {
            float A2 = e2.c.A((-f19) + AndroidUtilities.dp(4.0f), this.H, (1.0f - this.H) * (((-AndroidUtilities.dp(8.0f)) - (AndroidUtilities.dp2(8.0f) * f10)) - f15), f18);
            org.telegram.ui.Components.l5 l5Var2 = this.h0;
            if (l5Var2 == null || (imageReceiver = l5Var2.k) == null) {
                imageReceiver = this.C;
            }
            ImageReceiver imageReceiver2 = imageReceiver;
            imageReceiver2.setAlpha(f16);
            float f52 = f15 / 2.0f;
            imageReceiver2.setRoundRadius((int) f52);
            float f53 = f15;
            imageReceiver2.setImageCoords(f17 - f52, A2, f53, f53);
            if (this.U && imageReceiver2.getDrawable() != null && (imageReceiver2.getDrawable() instanceof org.telegram.ui.Components.mq) && (((org.telegram.ui.Components.mq) imageReceiver2.getDrawable()).b instanceof org.telegram.ui.Components.n80)) {
                ((org.telegram.ui.Components.n80) ((org.telegram.ui.Components.mq) imageReceiver2.getDrawable()).b).i = f10;
            }
            if (this.H <= 0.0f || !this.Y) {
                imageReceiver2.draw(canvas2);
            } else {
                canvas2.saveLayerAlpha(imageReceiver2.getImageX(), imageReceiver2.getImageY(), imageReceiver2.getImageX() + imageReceiver2.getImageWidth(), imageReceiver2.getImageY() + imageReceiver2.getImageHeight(), 255, 31);
                imageReceiver2.draw(canvas2);
                float f54 = this.H;
                org.telegram.ui.Components.sq sqVar3 = sqVar;
                sqVar3.e(sqVar3.s);
                canvas2.scale(f54, f54, (sqVar3.s / 2.0f) + sqVar3.A + AndroidUtilities.dp(12.0f) + f17, (A2 - AndroidUtilities.dp(6.0f)) + AndroidUtilities.dp(14.0f));
                canvas2.translate(AndroidUtilities.dp(12.0f) + f17, A2 - AndroidUtilities.dp(6.0f));
                float f55 = sqVar3.C;
                float f56 = sqVar3.l;
                if (f56 != 1.0f) {
                    f20 = 6.0f;
                    int i20 = sqVar3.c;
                    if (i20 == 0 || i20 == 1) {
                        rectF2 = rectF;
                        f21 = 14.0f;
                        sqVar3.e(sqVar3.s);
                        float dp14 = (sqVar3.x - AndroidUtilities.dp(f55 * 2.0f)) / 2.0f;
                        float f57 = sqVar3.B;
                        rectF2.set(f57, dp14, sqVar3.s + f57 + AndroidUtilities.dp(11.0f), AndroidUtilities.dp(23.0f) + dp14);
                    } else {
                        float f58 = f56 * 2.0f;
                        if (f58 > 1.0f) {
                            f58 = 1.0f;
                        }
                        float dp15 = (sqVar3.x - AndroidUtilities.dp(f55 * 2.0f)) / 2.0f;
                        int i21 = sqVar3.s;
                        int i22 = sqVar3.r;
                        if (i21 == i22) {
                            w10 = i21;
                            f21 = 14.0f;
                        } else {
                            f21 = 14.0f;
                            w10 = e2.c.w(1.0f, f58, i22, i21 * f58);
                        }
                        sqVar3.e(w10);
                        float f59 = sqVar3.B;
                        rectF2 = rectF;
                        rectF2.set(f59, dp15, w10 + f59 + AndroidUtilities.dp(11.0f), AndroidUtilities.dp(23.0f) + dp15);
                    }
                } else {
                    rectF2 = rectF;
                    f20 = 6.0f;
                    f21 = 14.0f;
                    sqVar3.e(sqVar3.s);
                    float dp16 = (sqVar3.x - AndroidUtilities.dp(f55 * 2.0f)) / 2.0f;
                    float f60 = sqVar3.B;
                    rectF2.set(f60, dp16, sqVar3.s + f60 + AndroidUtilities.dp(11.0f), AndroidUtilities.dp(23.0f) + dp16);
                }
                rectF2.inset(-AndroidUtilities.dp(2.0f), -AndroidUtilities.dp(2.0f));
                canvas2.drawRoundRect(rectF2, rectF2.height() / 2.0f, rectF2.height() / 2.0f, this.n);
                canvas2.restore();
                canvas2.save();
                float f61 = this.H;
                sqVar3.e(sqVar3.s);
                canvas2.scale(f61, f61, (sqVar3.s / 2.0f) + sqVar3.A + AndroidUtilities.dp(12.0f) + f17, (A2 - AndroidUtilities.dp(f20)) + AndroidUtilities.dp(f21));
                canvas2.translate(AndroidUtilities.dp(12.0f) + f17, A2 - AndroidUtilities.dp(f20));
                sqVar3.a(canvas2);
                canvas2.restore();
            }
            imageReceiver2.setAlpha(1.0f);
        }
        d("paintChatActionBackground").setAlpha(i15);
        org.telegram.ui.ActionBar.j6.h2.setAlpha(i12);
        textPaint2.setAlpha(alpha3);
        paint.setAlpha(alpha4);
    }

    public final void b(Canvas canvas, int i10, int i11) {
        int v02 = org.telegram.ui.ActionBar.j6.v0(org.telegram.ui.ActionBar.j6.Xk, this.g0);
        TextPaint textPaint = this.h;
        textPaint.setColor(v02);
        Paint d = d("paintChatComposeBackground");
        int alpha = d.getAlpha();
        int alpha2 = textPaint.getAlpha();
        d.setAlpha((int) (alpha * this.P));
        if (this.v != null) {
            float f10 = this.H;
            if (f10 < 1.0f) {
                textPaint.setAlpha((int) ((1.0f - f10) * alpha2 * this.P));
                float height = ((((i11 - i10) - this.v.getHeight()) / 2.0f) + i10) - (AndroidUtilities.dp(10.0f) * this.H);
                canvas.save();
                canvas.translate((this.c - this.y) / 2.0f, height);
                this.v.draw(canvas);
                canvas.restore();
            }
        }
        if (this.w != null) {
            float f11 = this.H;
            if (f11 > 0.0f) {
                textPaint.setAlpha((int) (alpha2 * f11 * this.P));
                float dp = ((1.0f - this.H) * AndroidUtilities.dp(10.0f)) + (((i11 - i10) - this.w.getHeight()) / 2.0f) + i10;
                canvas.save();
                canvas.translate((this.c - this.B) / 2.0f, dp);
                this.w.draw(canvas);
                canvas.restore();
            }
        }
        textPaint.setAlpha(alpha2);
        d.setAlpha(alpha);
    }

    public final Paint d(String str) {
        org.telegram.ui.ActionBar.f6 f6Var = this.g0;
        Paint G = f6Var != null ? f6Var.G(str) : null;
        return G != null ? G : org.telegram.ui.ActionBar.j6.S0(str);
    }

    @Override // org.telegram.messenger.NotificationCenter.NotificationCenterDelegate
    public final void didReceivedNotification(int i10, int i11, Object... objArr) {
        TLRPC.Dialog dialog;
        if (this.W == 0 || (dialog = (TLRPC.Dialog) MessagesController.getInstance(this.b0).dialogs_dict.f(this.W)) == null) {
            return;
        }
        int i12 = dialog.unread_count;
        this.Z.c(i12, true);
        this.Y = i12 > 0;
        View view = this.X;
        if (view != null) {
            view.invalidate();
        }
    }

    public final boolean e() {
        return this.P > 0.0f && !this.O;
    }

    public final void f() {
        View view;
        this.C.onAttachedToWindow();
        org.telegram.ui.Components.l5 l5Var = this.h0;
        if (l5Var != null && (view = this.X) != null) {
            l5Var.a(view);
        }
        NotificationCenter.getInstance(this.b0).addObserver(this, NotificationCenter.updateInterfaces);
    }

    public final void g(final View view, boolean z4) {
        AnimatorSet animatorSet = this.G;
        if (animatorSet != null) {
            animatorSet.removeAllListeners();
            this.G.cancel();
        }
        final int i10 = 1;
        final int i11 = 0;
        final int i12 = 2;
        if (!z4) {
            ValueAnimator ofFloat = ValueAnimator.ofFloat(this.H, 0.0f);
            final int i13 = 4;
            ofFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(this) { // from class: org.telegram.ui.tp
                public final /* synthetic */ vp b;

                {
                    this.b = this;
                }

                @Override // android.animation.ValueAnimator.AnimatorUpdateListener
                public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                    switch (i13) {
                        case 0:
                            vp vpVar = this.b;
                            vpVar.getClass();
                            vpVar.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                            view.invalidate();
                            vpVar.Q.invalidate();
                            break;
                        case 1:
                            vp vpVar2 = this.b;
                            vpVar2.getClass();
                            vpVar2.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                            view.invalidate();
                            break;
                        case 2:
                            vp vpVar3 = this.b;
                            vpVar3.getClass();
                            vpVar3.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                            view.invalidate();
                            break;
                        case 3:
                            vp vpVar4 = this.b;
                            vpVar4.getClass();
                            vpVar4.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                            view.invalidate();
                            break;
                        default:
                            vp vpVar5 = this.b;
                            vpVar5.getClass();
                            vpVar5.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                            vpVar5.Q.invalidate();
                            view.invalidate();
                            break;
                    }
                }
            });
            ofFloat.setInterpolator(org.telegram.ui.Components.mr.f);
            ofFloat.setDuration(220L);
            AnimatorSet animatorSet2 = new AnimatorSet();
            this.G = animatorSet2;
            animatorSet2.playTogether(ofFloat);
            this.G.start();
            return;
        }
        ValueAnimator ofFloat2 = ValueAnimator.ofFloat(this.H, 1.0f);
        ofFloat2.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(this) { // from class: org.telegram.ui.tp
            public final /* synthetic */ vp b;

            {
                this.b = this;
            }

            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                switch (i11) {
                    case 0:
                        vp vpVar = this.b;
                        vpVar.getClass();
                        vpVar.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        vpVar.Q.invalidate();
                        break;
                    case 1:
                        vp vpVar2 = this.b;
                        vpVar2.getClass();
                        vpVar2.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 2:
                        vp vpVar3 = this.b;
                        vpVar3.getClass();
                        vpVar3.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 3:
                        vp vpVar4 = this.b;
                        vpVar4.getClass();
                        vpVar4.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    default:
                        vp vpVar5 = this.b;
                        vpVar5.getClass();
                        vpVar5.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        vpVar5.Q.invalidate();
                        view.invalidate();
                        break;
                }
            }
        });
        ofFloat2.setInterpolator(org.telegram.ui.Components.mr.h);
        ofFloat2.setDuration(250L);
        this.I = 0.0f;
        ValueAnimator ofFloat3 = ValueAnimator.ofFloat(0.0f, 1.0f);
        ofFloat3.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(this) { // from class: org.telegram.ui.tp
            public final /* synthetic */ vp b;

            {
                this.b = this;
            }

            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                switch (i10) {
                    case 0:
                        vp vpVar = this.b;
                        vpVar.getClass();
                        vpVar.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        vpVar.Q.invalidate();
                        break;
                    case 1:
                        vp vpVar2 = this.b;
                        vpVar2.getClass();
                        vpVar2.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 2:
                        vp vpVar3 = this.b;
                        vpVar3.getClass();
                        vpVar3.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 3:
                        vp vpVar4 = this.b;
                        vpVar4.getClass();
                        vpVar4.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    default:
                        vp vpVar5 = this.b;
                        vpVar5.getClass();
                        vpVar5.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        vpVar5.Q.invalidate();
                        view.invalidate();
                        break;
                }
            }
        });
        org.telegram.ui.Components.mr mrVar = org.telegram.ui.Components.mr.j;
        ofFloat3.setInterpolator(mrVar);
        ofFloat3.setDuration(180L);
        ValueAnimator ofFloat4 = ValueAnimator.ofFloat(1.0f, -0.5f);
        ofFloat4.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(this) { // from class: org.telegram.ui.tp
            public final /* synthetic */ vp b;

            {
                this.b = this;
            }

            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                switch (i12) {
                    case 0:
                        vp vpVar = this.b;
                        vpVar.getClass();
                        vpVar.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        vpVar.Q.invalidate();
                        break;
                    case 1:
                        vp vpVar2 = this.b;
                        vpVar2.getClass();
                        vpVar2.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 2:
                        vp vpVar3 = this.b;
                        vpVar3.getClass();
                        vpVar3.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 3:
                        vp vpVar4 = this.b;
                        vpVar4.getClass();
                        vpVar4.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    default:
                        vp vpVar5 = this.b;
                        vpVar5.getClass();
                        vpVar5.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        vpVar5.Q.invalidate();
                        view.invalidate();
                        break;
                }
            }
        });
        ofFloat4.setInterpolator(mrVar);
        ofFloat4.setDuration(120L);
        ValueAnimator ofFloat5 = ValueAnimator.ofFloat(-0.5f, 0.0f);
        final int i14 = 3;
        ofFloat5.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(this) { // from class: org.telegram.ui.tp
            public final /* synthetic */ vp b;

            {
                this.b = this;
            }

            @Override // android.animation.ValueAnimator.AnimatorUpdateListener
            public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                switch (i14) {
                    case 0:
                        vp vpVar = this.b;
                        vpVar.getClass();
                        vpVar.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        vpVar.Q.invalidate();
                        break;
                    case 1:
                        vp vpVar2 = this.b;
                        vpVar2.getClass();
                        vpVar2.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 2:
                        vp vpVar3 = this.b;
                        vpVar3.getClass();
                        vpVar3.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    case 3:
                        vp vpVar4 = this.b;
                        vpVar4.getClass();
                        vpVar4.I = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        view.invalidate();
                        break;
                    default:
                        vp vpVar5 = this.b;
                        vpVar5.getClass();
                        vpVar5.H = ((Float) valueAnimator.getAnimatedValue()).floatValue();
                        vpVar5.Q.invalidate();
                        view.invalidate();
                        break;
                }
            }
        });
        ofFloat5.setInterpolator(mrVar);
        ofFloat5.setDuration(100L);
        AnimatorSet animatorSet3 = new AnimatorSet();
        this.G = animatorSet3;
        animatorSet3.addListener(new dg.y2(12, this, view));
        AnimatorSet animatorSet4 = new AnimatorSet();
        animatorSet4.playSequentially(ofFloat3, ofFloat4, ofFloat5);
        this.G.playTogether(ofFloat2, animatorSet4);
        this.G.start();
    }

    public final void h() {
        this.S = false;
        this.E = null;
        TLRPC.Dialog c3 = c(this.f0, this.c0, this.d0, true, this.a0);
        if (c3 == null) {
            this.D = null;
            this.T = false;
            this.O = true;
            return;
        }
        this.W = c3.id;
        int[] iArr = this.a0;
        this.T = iArr[0] == 1;
        this.a = iArr[1];
        this.b = iArr[2];
        this.O = false;
        int i10 = this.b0;
        TLRPC.Chat chat = MessagesController.getInstance(i10).getChat(Long.valueOf(-c3.id));
        this.D = chat;
        if (chat == null) {
            this.D = MessagesController.getInstance(i10).getChat(Long.valueOf(c3.id));
        }
        org.telegram.ui.Components.z8 z8Var = new org.telegram.ui.Components.z8((org.telegram.ui.ActionBar.f6) null);
        z8Var.k(i10, this.D);
        this.C.setImage(ImageLocation.getForChat(this.D, 1), "50_50", z8Var, null, UserConfig.getInstance(0).getCurrentUser(), 0);
        MessagesController.getInstance(i10).ensureMessagesLoaded(c3.id, 0, null);
        int i11 = c3.unread_count;
        this.Z.c(i11, false);
        this.Y = i11 > 0;
    }

    public final void i(TLRPC.Chat chat) {
        if (chat == null) {
            h();
            return;
        }
        this.W = -chat.id;
        int[] iArr = this.a0;
        this.T = iArr[0] == 1;
        this.a = iArr[1];
        this.b = iArr[2];
        this.O = false;
        this.D = chat;
        org.telegram.ui.Components.z8 z8Var = new org.telegram.ui.Components.z8((org.telegram.ui.ActionBar.f6) null);
        TLRPC.Chat chat2 = this.D;
        int i10 = this.b0;
        z8Var.k(i10, chat2);
        this.C.setImage(ImageLocation.getForChat(this.D, 1), "50_50", z8Var, null, UserConfig.getInstance(0).getCurrentUser(), 0);
        MessagesController.getInstance(i10).ensureMessagesLoaded(-chat.id, 0, null);
        TLRPC.Dialog dialog = MessagesController.getInstance(i10).getDialog(-chat.id);
        int i11 = dialog == null ? 0 : dialog.unread_count;
        this.Z.c(i11, false);
        this.Y = i11 > 0;
        this.S = true;
        this.E = null;
    }

    public final void j() {
        TLRPC.TL_forumTopic tL_forumTopic;
        org.telegram.ui.Components.l5 l5Var;
        View view;
        View view2;
        org.telegram.ui.Components.l5 l5Var2;
        TLRPC.Message message;
        TLRPC.Message message2;
        this.S = false;
        this.T = false;
        this.D = null;
        this.W = 0L;
        ImageReceiver imageReceiver = this.C;
        imageReceiver.clearImage();
        long j10 = -this.f0;
        int i10 = this.b0;
        ArrayList<TLRPC.TL_forumTopic> topics = MessagesController.getInstance(i10).getTopicsController().getTopics(j10);
        if (topics == null || topics.size() <= 1) {
            tL_forumTopic = null;
        } else {
            tL_forumTopic = null;
            for (int i11 = 0; i11 < topics.size(); i11++) {
                TLRPC.TL_forumTopic tL_forumTopic2 = topics.get(i11);
                if (tL_forumTopic2.id != this.e0 && !tL_forumTopic2.hidden && tL_forumTopic2.unread_count > 0 && (tL_forumTopic == null || ((message = tL_forumTopic2.topMessage) != null && (message2 = tL_forumTopic.topMessage) != null && message.date > message2.date))) {
                    tL_forumTopic = tL_forumTopic2;
                }
            }
        }
        if (tL_forumTopic == null) {
            this.E = null;
            this.O = true;
            return;
        }
        this.O = false;
        this.E = tL_forumTopic;
        int i12 = tL_forumTopic.id;
        org.telegram.ui.ActionBar.f6 f6Var = this.g0;
        if (i12 == 1) {
            View view3 = this.X;
            if (view3 != null && (l5Var2 = this.h0) != null) {
                l5Var2.o(view3);
            }
            this.h0 = null;
            imageReceiver.setImageBitmap(ag.f.c(this.Q.getContext(), 1.0f, org.telegram.ui.ActionBar.j6.v0(org.telegram.ui.ActionBar.j6.Ac, f6Var), true));
        } else if (tL_forumTopic.icon_emoji_id != 0) {
            org.telegram.ui.Components.l5 l5Var3 = this.h0;
            if (l5Var3 == null || l5Var3.i() != tL_forumTopic.icon_emoji_id) {
                org.telegram.ui.Components.l5 l5Var4 = this.h0;
                if (l5Var4 != null && (view = this.X) != null) {
                    l5Var4.o(view);
                }
                org.telegram.ui.Components.l5 l5Var5 = new org.telegram.ui.Components.l5(22, i10, tL_forumTopic.icon_emoji_id);
                this.h0 = l5Var5;
                l5Var5.setColorFilter(new PorterDuffColorFilter(org.telegram.ui.ActionBar.j6.v0(org.telegram.ui.ActionBar.j6.ic, f6Var), PorterDuff.Mode.SRC_IN));
            }
            org.telegram.ui.Components.l5 l5Var6 = this.h0;
            if (l5Var6 != null && (view2 = this.X) != null) {
                l5Var6.a(view2);
            }
            imageReceiver.setImageBitmap((Bitmap) null);
        } else {
            View view4 = this.X;
            if (view4 != null && (l5Var = this.h0) != null) {
                l5Var.o(view4);
            }
            this.h0 = null;
            imageReceiver.setImageBitmap(ag.f.e(tL_forumTopic));
        }
        int i13 = tL_forumTopic.unread_count;
        this.Z.c(i13, false);
        this.Y = i13 > 0;
    }
}
