...
public class BalanceCloud extends LinearLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    public BalanceCloud(final Context context, int i, long j, AmountUtils$Currency amountUtils$Currency, final Theme.ResourcesProvider resourcesProvider) {
        ...
        linksTextView.setText(AndroidUtilities.replaceArrows(AndroidUtilities.replaceSingleTag(LocaleController.getString(R.string.Gift2MessageStarsInfoLink), new Runnable() { // from class: org.telegram.ui.Stars.BalanceCloud$$ExternalSyntheticLambda1
    ...
    private void updateBalance(boolean z) {
        ...
        if (amountUtils$Currency == AmountUtils$Currency.STARS) {
            ...
            this.textView2.setText(AndroidUtilities.replaceArrows(AndroidUtilities.replaceSingleTag(LocaleController.getString(R.string.Gift2MessageStarsInfoLink), new Runnable() { // from class: org.telegram.ui.Stars.BalanceCloud$$ExternalSyntheticLambda0
                @Override // java.lang.Runnable
                public final void run() {
        ...
        if (amountUtils$Currency == AmountUtils$Currency.TON) {
            ...
            if (this.chatId == -1) {
                formatString = LocaleController.formatString(R.string.Gift2MessageStarsInfoTON, balanceAmount.asDecimalString());
            } else {
                formatString = LocaleController.formatString(R.string.Gift2MessageChannelStarsInfoTON, AmountUtils$Amount$$ExternalSyntheticBackportWithForwarding0.m(new BigDecimal(BotStarsController.getInstance(this.currentAccount).getBotStarsBalance(-this.chatId).amount).divide(BigDecimal.valueOf(1000000000L), MathContext.UNLIMITED)).toPlainString());
            }
            textView2.setText(StarsIntroActivity.replaceStarsWithPlain(true, (CharSequence) AndroidUtilities.replaceTags(formatString), 0.6f, this.coloredImageSpansTon));
