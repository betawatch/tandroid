package org.telegram.ui;

import java.util.ArrayList;
import org.telegram.messenger.MessageObject;

/* compiled from: r8-map-id-e506a87262d42a59d49ceeb11de21243ca58d8dd989db9ff2eb23aa08d8dd348 */
/* loaded from: classes3.dex */
public final /* synthetic */ class t91 implements Runnable {
    public final /* synthetic */ int a;
    public final /* synthetic */ za1 b;
    public final /* synthetic */ ArrayList c;

    public /* synthetic */ t91(za1 za1Var, ArrayList arrayList, int i10) {
        this.a = i10;
        this.b = za1Var;
        this.c = arrayList;
    }

    @Override // java.lang.Runnable
    public final void run() {
        boolean z10;
        switch (this.a) {
            case 0:
                za1 za1Var = this.b;
                ArrayList arrayList = za1Var.s0;
                ArrayList arrayList2 = za1Var.r0;
                int i10 = 0;
                za1Var.w0 = false;
                ArrayList arrayList3 = this.c;
                if (!arrayList3.isEmpty()) {
                    int size = arrayList3.size();
                    for (int i11 = 0; i11 < size; i11++) {
                        MessageObject messageObject = (MessageObject) arrayList3.get(i11);
                        int i12 = za1Var.p0.get(messageObject.getId(), -1);
                        if (i12 >= 0 && ((wa1) arrayList2.get(i12)).b() == messageObject.getId()) {
                            ((wa1) arrayList2.get(i12)).b = messageObject;
                        }
                    }
                    arrayList.clear();
                    int size2 = arrayList2.size();
                    while (true) {
                        if (i10 < size2) {
                            wa1 wa1Var = (wa1) arrayList2.get(i10);
                            if (wa1Var.b == null) {
                                za1Var.o0 = wa1Var.b();
                            } else {
                                arrayList.add(wa1Var);
                                i10++;
                            }
                        }
                    }
                    za1Var.o0();
                    za1Var.S.setItemAnimator(null);
                    za1Var.y0.f();
                    break;
                }
                break;
            default:
                za1 za1Var2 = this.b;
                ai.d9 d9Var = za1Var2.z0;
                d9Var.getClass();
                ArrayList arrayList4 = this.c;
                int size3 = arrayList4.size();
                int i13 = 0;
                while (true) {
                    if (i13 < size3) {
                        Object obj = arrayList4.get(i13);
                        i13++;
                        if (!d9Var.j.containsKey((Integer) obj)) {
                            z10 = true;
                        }
                    } else {
                        z10 = false;
                    }
                }
                if (!d9Var.q(0, arrayList4, z10)) {
                    za1Var2.j0();
                    za1Var2.o0();
                    break;
                }
                break;
        }
    }
}
