package org.telegram.ui;

import android.graphics.drawable.Drawable;
import android.view.View;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.ImageReceiver;
import org.telegram.tgnet.TLRPC;

/* compiled from: r8-map-id-31c59681dc67c50f9c85463306fa4201c22270b60fa73c0aa0aee7d630a89c77 */
/* loaded from: classes3.dex */
public final /* synthetic */ class qb1 implements ImageReceiver.ImageReceiverDelegate, org.telegram.ui.Components.jl0, org.telegram.ui.Components.n81, org.telegram.ui.ActionBar.c2 {
    public final /* synthetic */ int a;
    public final /* synthetic */ cd1 b;

    public /* synthetic */ qb1(cd1 cd1Var, int i10) {
        this.a = i10;
        this.b = cd1Var;
    }

    @Override // org.telegram.ui.Components.jl0
    public void c(float f10, float f11, int i10, View view) {
        if (view instanceof org.telegram.ui.Cells.t1) {
            org.telegram.ui.Cells.t1 t1Var = (org.telegram.ui.Cells.t1) view;
            boolean i32 = t1Var.i3(f10);
            cd1 cd1Var = this.b;
            if (!i32) {
                cd1Var.Y0(2, true);
            } else if (t1Var.getMessageObject().isOutOwner()) {
                cd1Var.Y0(3, true);
            } else {
                cd1Var.Y0(1, true);
            }
        }
    }

    @Override // org.telegram.ui.Components.n81
    public void d(int i10, int i11) {
        cd1 cd1Var = this.b;
        if (cd1Var.B1) {
            cd1Var.u0.getBackground();
            float scaleX = cd1Var.y0 != null ? (cd1Var.u0.getScaleX() - 1.0f) / (cd1Var.v1 - 1.0f) : 1.0f;
            cd1Var.u0.setTranslationX(i10 * scaleX);
            cd1Var.u0.setTranslationY(i11 * scaleX);
        }
    }

    @Override // org.telegram.messenger.ImageReceiver.ImageReceiverDelegate
    public void didSetImage(ImageReceiver imageReceiver, boolean z4, boolean z10, boolean z11) {
        cd1 cd1Var = this.b;
        if (cd1Var.y1 instanceof mi1) {
            return;
        }
        Drawable drawable = imageReceiver.getDrawable();
        if (!z4 || drawable == null) {
            return;
        }
        cc1 cc1Var = cd1Var.a;
        AndroidUtilities.calcDrawableColor(drawable);
        cc1Var.b(cd1Var.P0(drawable), drawable, Float.valueOf(cd1Var.i1));
        if (!z10 && cd1Var.C1 && cd1Var.t1 == null) {
            cd1Var.u0.getImageReceiver().setCrossfadeWithOldImage(false);
            cd1Var.i1();
            cd1Var.u0.getImageReceiver().setCrossfadeWithOldImage(true);
        }
        cd1Var.V0();
    }

    @Override // org.telegram.messenger.ImageReceiver.ImageReceiverDelegate
    public /* synthetic */ void didSetImageBitmap(int i10, String str, Drawable drawable) {
        org.telegram.messenger.j5.a(this, i10, str, drawable);
    }

    @Override // org.telegram.ui.Components.jl0
    public /* synthetic */ boolean e1(View view) {
        return false;
    }

    @Override // org.telegram.ui.ActionBar.c2
    public void i(org.telegram.ui.ActionBar.d2 d2Var, int i10) {
        switch (this.a) {
            case 3:
                this.b.p0.getActionBarMenuOnItemClick().b(4);
                break;
            case 4:
                this.b.O0(false);
                break;
            case 5:
                cd1 cd1Var = this.b;
                org.telegram.ui.ActionBar.h6 h6Var = cd1Var.s;
                if (h6Var.j == 4294967296L) {
                    h6Var.j = 0L;
                    h6Var.k = 0L;
                    h6Var.l = 0L;
                    h6Var.m = 0L;
                    cd1Var.m1(false);
                    org.telegram.ui.ActionBar.j6.n1(false, false);
                }
                cd1Var.v = true;
                org.telegram.ui.ActionBar.j6.p1(true);
                cd1Var.Y0(2, false);
                break;
            case 6:
                cd1 cd1Var2 = this.b;
                org.telegram.ui.ActionBar.h6 h6Var2 = cd1Var2.s;
                if (org.telegram.ui.ActionBar.j6.Z0() && org.telegram.ui.ActionBar.j6.I.f0.d != 0) {
                    org.telegram.ui.ActionBar.c6 c6Var = h6Var2.y;
                    h6Var2.j = c6Var.d;
                    h6Var2.k = c6Var.e;
                    h6Var2.l = c6Var.f;
                    h6Var2.m = c6Var.g;
                    h6Var2.n = c6Var.h;
                    String str = c6Var.c;
                    h6Var2.o = str;
                    float f10 = c6Var.k;
                    h6Var2.p = f10;
                    cd1Var2.i1 = f10;
                    if (str == null || "c".equals(str)) {
                        cd1Var2.T0 = null;
                    } else {
                        int size = cd1Var2.R0.size();
                        int i11 = 0;
                        while (true) {
                            if (i11 < size) {
                                TLRPC.TL_wallPaper tL_wallPaper = (TLRPC.TL_wallPaper) cd1Var2.R0.get(i11);
                                if (tL_wallPaper.pattern && h6Var2.o.equals(tL_wallPaper.slug)) {
                                    cd1Var2.T0 = tL_wallPaper;
                                } else {
                                    i11++;
                                }
                            }
                        }
                    }
                    cd1Var2.v = true;
                    cd1Var2.G0[1].a(cd1Var2.T0 != null, true);
                    cd1Var2.m1(false);
                    org.telegram.ui.ActionBar.j6.n1(false, false);
                }
                Drawable background = cd1Var2.u0.getBackground();
                if (background instanceof org.telegram.ui.Components.dc0) {
                    org.telegram.ui.Components.dc0 dc0Var = (org.telegram.ui.Components.dc0) background;
                    dc0Var.t(null, 100);
                    if (org.telegram.ui.ActionBar.j6.I.q()) {
                        if (cd1Var2.i1 < 0.0f) {
                            cd1Var2.u0.getImageReceiver().setGradientBitmap(dc0Var.k);
                        }
                        org.telegram.ui.Cells.j0 j0Var = cd1Var2.Q0;
                        if (j0Var != null) {
                            j0Var.setTwoSided(true);
                        }
                    } else {
                        float f11 = cd1Var2.i1;
                        if (f11 < 0.0f) {
                            cd1Var2.i1 = -f11;
                        }
                    }
                }
                org.telegram.ui.Cells.j0 j0Var2 = cd1Var2.Q0;
                if (j0Var2 != null) {
                    j0Var2.setProgress(cd1Var2.i1);
                }
                org.telegram.ui.ActionBar.j6.p1(true);
                cd1Var2.Y0(2, false);
                break;
            default:
                cd1 cd1Var3 = this.b;
                org.telegram.ui.ActionBar.h6 h6Var3 = cd1Var3.s;
                if (h6Var3.j == 4294967296L) {
                    h6Var3.j = 0L;
                    h6Var3.k = 0L;
                    h6Var3.l = 0L;
                    h6Var3.m = 0L;
                    cd1Var3.m1(false);
                    org.telegram.ui.ActionBar.j6.n1(false, false);
                }
                cd1Var3.v = true;
                org.telegram.ui.ActionBar.j6.p1(true);
                cd1Var3.Y0(2, false);
                break;
        }
    }

    @Override // org.telegram.messenger.ImageReceiver.ImageReceiverDelegate
    public /* synthetic */ void onAnimationReady(ImageReceiver imageReceiver) {
        org.telegram.messenger.j5.b(this, imageReceiver);
    }

    @Override // org.telegram.ui.Components.jl0
    public /* synthetic */ void o0(View view, float f10, float f11) {
    }
}
