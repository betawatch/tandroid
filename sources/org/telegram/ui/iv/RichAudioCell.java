...
public class RichAudioCell extends RichBlockCell implements Theme.Colorable, TextSelectionHelper.ArticleSelectableView, RichCaptionHost, NotificationCenter.NotificationCenterDelegate, DownloadController.FileDownloadProgressListener {
    ...
    public RichAudioCell(Context context, int i, Theme.ResourcesProvider resourcesProvider) {
        ...
        RichCaptionController richCaptionController = new RichCaptionController(context, resourcesProvider, new RichCaptionController.Host() { // from class: org.telegram.ui.iv.RichAudioCell.2
    ...
    private void layoutInner() {
        ...
        if (!TextUtils.isEmpty(audioTitle) || !TextUtils.isEmpty(audioAuthor)) {
            if (!TextUtils.isEmpty(audioTitle) && !TextUtils.isEmpty(audioAuthor)) {
                spannableStringBuilder = new SpannableStringBuilder(String.format("%s - %s", audioAuthor, audioTitle));
            } else if (!TextUtils.isEmpty(audioTitle)) {
                spannableStringBuilder = new SpannableStringBuilder(audioTitle);
            } else {
                spannableStringBuilder = new SpannableStringBuilder(audioAuthor);
            }
            if (!TextUtils.isEmpty(audioAuthor)) {
                spannableStringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.bold()), 0, audioAuthor.length(), 18);
            }
    ...
    private int audioDuration() {
        ...
        TLRPC.TL_documentAttributeAudio attribute = attribute();
        if (attribute != null) {
    ...
    public void updatePlayingMessageProgress() {
        ...
        String formatShortDuration = AndroidUtilities.formatShortDuration(isUploading() ? attribute() != null ? (int) attribute().duration : 0 : audioDuration());
