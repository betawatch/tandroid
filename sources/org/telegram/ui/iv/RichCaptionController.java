...
class RichCaptionController {
    ...
    RichCaptionController(Context context, Theme.ResourcesProvider resourcesProvider, final Host host) {
        ...
        richEditText.setListener(new 1(host));
    ...
    class 1 implements RichEditText.Listener {
        ...
        public void onSelectionChanged(final RichEditText richEditText, final int i, final int i2) {
            ...
            if (RichCaptionController.this.hijackingSelection || i == i2 || (selectionHelper = this.val$host.selectionHelper()) == null) {
                return;
            }
            if (selectionHelper.isInSelectionMode() && selectionHelper.getSelectedCell() == this.val$host.cell()) {
                return;
            }
    ...
    void bind() {
        ...
        BlockRow currentRow = this.host.currentRow();
        if (currentRow == null || (pageBlock = currentRow.block) == null) {
            return;
        }
        ensureCaption(pageBlock);
        TL_iv.RichText richText = currentRow.block.caption.text;
    ...
    void persist() {
        ...
        ensureCaption(pageBlock);
        currentRow.block.caption.text = RichTextStyle.fromSpannable(this.editText.getText());
    }
    ...
    int measure(int i, int i2, int i3) {
        this.editText.measure(View.MeasureSpec.makeMeasureSpec(Math.max(0, ((i3 - i) - i2) - (AndroidUtilities.dp(16.0f) * 2)), TLObject.FLAG_30), View.MeasureSpec.makeMeasureSpec(0, 0));
        return this.editText.getMeasuredHeight();
    }

    void layout(int i, int i2, int i3, int i4) {
        int dp = i + AndroidUtilities.dp(16.0f);
        this.editText.layout(dp, i4, Math.max(dp, (i3 - i2) - AndroidUtilities.dp(16.0f)), this.editText.getMeasuredHeight() + i4);
    }
    ...
    void fillTextLayoutBlocks(ArrayList arrayList) {
        ...
        arrayList.add(new TextSelectionHelper.TextLayoutBlock() { // from class: org.telegram.ui.iv.RichCaptionController.2
    ...
    void drawSelection(Canvas canvas) {
        TextSelectionHelper.ArticleTextSelectionHelper selectionHelper = this.host.selectionHelper();
        ...
        selectionHelper.draw(canvas, this.host.cell(), 0);
        canvas.restore();
    }
