...
public class RichMediaConverter implements NotificationCenter.NotificationCenterDelegate {
    ...
    private static VideoEditedInfo buildVideoEditedInfo(MediaController.PhotoEntry photoEntry) {
        ...
        float f = i;
        float f2 = i2;
        float max = Math.max(f / 854.0f, f2 / 854.0f);
        if (max < 1.0f) {
            max = 1.0f;
        }
        int i9 = (int) (f / max);
        ...
        if (i10 % 16 != 0) {
            i10 = Math.max(1, Math.round(i10 / 16.0f)) * 16;
        }
    ...
    public void start() {
        ...
        this.started = true;
        VideoEditedInfo buildVideoEditedInfo = buildVideoEditedInfo(this.entry);
        this.info = buildVideoEditedInfo;
        if (buildVideoEditedInfo == null || !buildVideoEditedInfo.needConvert()) {
            fail();
            return;
        }
    ...
    public void didReceivedNotification(int i, int i2, Object... objArr) {
        ...
        if (i == NotificationCenter.fileNewChunkAvailable) {
            ...
            if (listener != null) {
                listener.onProgress(floatValue);
            }
            if (longValue > 0) {
                this.finished = true;
                teardown();
