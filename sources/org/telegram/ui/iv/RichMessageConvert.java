...
public abstract class RichMessageConvert {
    ...
    public static ArrayList blocksFromCharSequence(CharSequence charSequence) {
        ...
        int[] lineStarts = lineStarts(charSequence);
        ...
        while (i < lineStarts.length) {
            ...
            if (span == null && quoteSpan == 0) {
                ...
                while (i < lineStarts.length) {
                    int i4 = lineStarts[i];
                    int i5 = i + 1;
                    int i6 = i5 < lineStarts.length ? lineStarts[i5] - 1 : length;
                    if (span != null) {
                        blockSpan = blockSpan(spanned, i4, i6, CodeHighlighting.Span.class);
                    } else {
                ...
                if (span != null) {
                    ...
                    pageblockpreformatted.text = RichTextStyle.fromSpannable(subSequence);
                    String str = span.lng;
                    if (str == null) {
                        str = "";
                    }
    ...
    private static void collectBlocks(ArrayList arrayList, List list) {
        ...
        while (it.hasNext()) {
            CharSequence renderBlock = renderBlock((TL_iv.PageBlock) it.next());
            if (renderBlock != null) {
    ...
    private static CharSequence rowsToCharSequence(List list, boolean z) {
        ...
        while (list != null && i < list.size()) {
            ...
            if (!blockRow.quoteIds.isEmpty()) {
                long longValue = ((Long) blockRow.quoteIds.get(0)).longValue();
                SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder();
                int i2 = i;
                while (i2 < list.size() && !((BlockRow) list.get(i2)).quoteIds.isEmpty() && ((Long) ((BlockRow) list.get(i2)).quoteIds.get(0)).longValue() == longValue) {
                    if (i2 > i) {
                        spannableStringBuilder.append('\n');
                    }
