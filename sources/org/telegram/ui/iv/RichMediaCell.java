...
public class RichMediaCell extends FrameLayout implements Theme.Colorable, TextSelectionHelper.ArticleSelectableView, RichCaptionHost {
    ...
    public RichMediaCell(Context context, Theme.ResourcesProvider resourcesProvider) {
        ...
        if (z && i >= 31) {
            ...
            this.blurColors = new BlurredBackgroundColorProvider() { // from class: org.telegram.ui.iv.RichMediaCell.2
        ...
        this.addButton = createCircleButton;
        createCircleButton.setImageResource(R.drawable.iv_media_add);
        addView(createCircleButton, LayoutHelper.createFrame(32, 32.0f, 53, 12.0f, 12.0f, 12.0f, 12.0f));
        ...
        this.switchModeButton = createCircleButton2;
        createCircleButton2.setVisibility(8);
        addView(createCircleButton2, LayoutHelper.createFrame(32, 32.0f, 53, 12.0f, 12.0f, 66.0f, 12.0f));
    ...
    private void rebuildItems() {
        List medias = medias();
        while (this.items.size() < medias.size()) {
        ...
        for (int i = 0; i < medias.size(); i++) {
            ((RichMediaItem) this.items.get(i)).setMedia((MediaUploadState) medias.get(i));
        }
        while (this.deleteButtons.size() < medias.size()) {
            ImageView createCircleButton = createCircleButton();
            createCircleButton.setImageResource(R.drawable.iv_media_delete);
            ...
            this.deleteButtons.add(createCircleButton);
        }
    ...
    private void onDeleteClicked(int i) {
        ...
        this.delegate.onDeleteMedia(this.currentRow, (MediaUploadState) medias.get(i));
    }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        for (int i3 = 0; i3 < this.deleteButtons.size(); i3++) {
            ((ImageView) this.deleteButtons.get(i3)).measure(View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(32.0f), TLObject.FLAG_30), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(32.0f), TLObject.FLAG_30));
        }
    ...
    private void computeGeometry(int i) {
        ...
        if (size == 1) {
            ...
            if (width > 0 && height > 0) {
                dp = (int) ((i / Math.max(1, width)) * height);
                Point point = AndroidUtilities.displaySize;
                int max = (int) (Math.max(point.x, point.y) * 0.55f);
                if (dp > max) {
                    i2 = (int) ((max / Math.max(1, height)) * width);
                    dp = max;
                    this.collageRects.add(new RectF((i - i2) / 2, paddingTop, r1 + i2, paddingTop + dp));
                    this.collageH = dp;
                    this.slideW = i2;
                    this.slideH = dp;
                    return;
        ...
        for (int i4 = 0; i4 < size; i4++) {
            fArr[i4] = aspectRatio((RichMediaItem) this.items.get(i4));
        }
        ...
        for (MessageObject.GroupedMessagePosition groupedMessagePosition : computeGrouped) {
            i5 = Math.max(i5, (int) groupedMessagePosition.maxY);
        }
        int i6 = i5 + 1;
        ...
        int i7 = 0;
        while (i7 < length) {
            MessageObject.GroupedMessagePosition groupedMessagePosition3 = computeGrouped[i7];
        ...
        while (i13 < computeGrouped.length) {
            MessageObject.GroupedMessagePosition groupedMessagePosition4 = computeGrouped[i13];
            ...
            if (groupedMessagePosition4.leftSpanOffset > 0) {
                ...
                for (int i17 = 0; i17 < computeGrouped.length; i17++) {
                    if (i17 != i13) {
                        MessageObject.GroupedMessagePosition groupedMessagePosition5 = computeGrouped[i17];
                        ...
                        if (b3 <= b4 && groupedMessagePosition5.maxY >= b4 && groupedMessagePosition5.minX < groupedMessagePosition4.minX) {
                            i16 += groupedMessagePosition5.pw;
                        }
                    }
                }
            ...
            this.collageRects.add(new RectF(round, i14 + paddingTop, round + Math.max(0, round2), r12 + Math.max(0, i15)));
            i13++;
            paddingTop = paddingTop;
            computeGrouped = computeGrouped;
        }
        ...
        for (int i8 = 0; i8 < this.deleteButtons.size(); i8++) {
            ...
            if (i8 >= medias.size() || ((MediaUploadState) medias.get(i8)).state == 0 || i8 >= this.itemRects.size()) {
                ...
                if (rectF.right <= 0.0f || rectF.left >= i5 || rectF.bottom <= getPaddingTop() || rectF.top >= getPaddingTop() + this.imageH) {
                    ...
                    imageView2.setVisibility(f > 0.01f ? 0 : 8);
                }
            }
        }
    }
    ...
    private void settle() {
        ...
        this.settleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.iv.RichMediaCell$$ExternalSyntheticLambda0
            ...
            public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                RichMediaCell.this.lambda$settle$3(valueAnimator);
            }
        });
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$settle$3(ValueAnimator valueAnimator) {
    ...
    private void handleTap(int i) {
        ...
        List medias = medias();
        if (medias.isEmpty() || (medias.size() == 1 && ((MediaUploadState) medias.get(0)).state == 0)) {
            this.delegate.onMediaPick(this.currentRow);
        } else {
            if (i < 0 || i >= medias.size() || !((MediaUploadState) medias.get(i)).isPending()) {
                return;
            }
            this.delegate.onCancelUpload(this.currentRow, (MediaUploadState) medias.get(i));
        }
    }
