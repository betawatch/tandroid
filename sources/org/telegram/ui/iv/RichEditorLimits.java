...
public class RichEditorLimits {
    ...
    private static void measureBlock(TL_iv.PageBlock pageBlock, int i, RichEditorLimits richEditorLimits) {
        ...
        if (pageBlock instanceof TL_iv.pageBlockList) {
            ...
            while (i3 < pageblocklist.items.size()) {
                measureListItem(pageblocklist.items.get(i3), i + 1, richEditorLimits);
                i3++;
            }
            return;
        }
        if (pageBlock instanceof TL_iv.pageBlockOrderedList) {
            ...
            while (i3 < pageblockorderedlist.items.size()) {
                measureOrderedItem(pageblockorderedlist.items.get(i3), i + 1, richEditorLimits);
                i3++;
            }
            return;
        }
        if (pageBlock instanceof TL_iv.pageBlockTable) {
            ...
            if (arrayList != null) {
                richEditorLimits.blocks += arrayList.size();
                for (int i4 = 0; i4 < pageblocktable.rows.size(); i4++) {
                    TL_iv.pageTableRow pagetablerow = pageblocktable.rows.get(i4);
                    if (pagetablerow.cells != null) {
                        i2 = 0;
                        for (int i5 = 0; i5 < pagetablerow.cells.size(); i5++) {
                            TL_iv.pageTableCell pagetablecell = pagetablerow.cells.get(i5);
                            i2 += TableModel.spanCol(pagetablecell);
        ...
        if (pageBlock instanceof TL_iv.pageBlockCollage) {
            ...
            while (i3 < arrayList3.size()) {
                measureBlock(arrayList3.get(i3), i + 1, richEditorLimits);
                i3++;
            }
            return;
        }
        if (pageBlock instanceof TL_iv.pageBlockSlideshow) {
            ...
            while (i3 < arrayList4.size()) {
                measureBlock(arrayList4.get(i3), i + 1, richEditorLimits);
                i3++;
            }
            return;
        }
    ...
    private static void addText(TL_iv.RichText richText, int i, RichEditorLimits richEditorLimits) {
        ...
        int richTextDepth = i + richTextDepth(richText);
        if (richTextDepth > richEditorLimits.depth) {
            richEditorLimits.depth = richTextDepth;
        }
    }
