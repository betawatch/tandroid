...
class VerticalBox extends Box {
    ...
    public void draw(Graphics2D graphics2D, float f9, float f10) {
        ...
        while (it.hasNext()) {
            ...
            next.draw(graphics2D, (next.getShift() + f9) - this.leftMostPos, height);
    ...
    public VerticalBox(Box box, float f9, int i10) {
        this();
        add(box);
        if (i10 == 2) {
            float f10 = f9 / 2.0f;
        ...
        if (i10 == 3) {
            this.depth += f9;
            super.add(new StrutBox(0.0f, f9, 0.0f, 0.0f));
        } else if (i10 == 4) {
            this.height += f9;
            super.add(0, new StrutBox(0.0f, f9, 0.0f, 0.0f));
        }
    }

    public final void add(Box box, float f9) {
        if (this.children.size() >= 1) {
