package t3;

import f5.w;
import j3.s0;
import j3.t0;
import j3.t1;
import o3.h;
import o3.k;
import o3.l;
import o3.m;
import o3.n;
import o3.o;
import org.xmlpull.v1.XmlPullParserException;
import q8.l0;
import w3.j;

/* compiled from: r8-map-id-53ae6996d745fb61649afae8ef429049dda227e2aa02488fda2c3b459d1c2b94 */
/* loaded from: classes.dex */
public final class a implements k {
    public m b;
    public int c;
    public int d;
    public int e;
    public h4.b g;
    public l h;
    public f2.c i;
    public w3.l j;
    public final w a = new w(6);
    public long f = -1;

    public final void a() {
        b(new b4.b[0]);
        m mVar = this.b;
        mVar.getClass();
        mVar.e1();
        this.b.D1(new o(-9223372036854775807L));
        this.c = 6;
    }

    public final void b(b4.b... bVarArr) {
        m mVar = this.b;
        mVar.getClass();
        o3.w Z1 = mVar.Z1(1024, 4);
        s0 s0Var = new s0();
        s0Var.n = "image/jpeg";
        s0Var.i = new b4.c(bVarArr);
        Z1.b(new t0(s0Var));
    }

    /* JADX WARN: Removed duplicated region for block: B:53:0x0166  */
    @Override // o3.k
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public final int d(l lVar, n nVar) {
        String m10;
        f2.c cVar;
        l0 l0Var;
        int i10;
        h4.b bVar;
        long j10;
        int i11 = this.c;
        w wVar = this.a;
        if (i11 == 0) {
            wVar.z(2);
            ((h) lVar).c(wVar.a, 0, 2, false);
            int w10 = wVar.w();
            this.d = w10;
            if (w10 == 65498) {
                if (this.f != -1) {
                    this.c = 4;
                    return 0;
                }
                a();
                return 0;
            }
            if ((w10 < 65488 || w10 > 65497) && w10 != 65281) {
                this.c = 1;
            }
            return 0;
        }
        if (i11 == 1) {
            wVar.z(2);
            ((h) lVar).c(wVar.a, 0, 2, false);
            this.e = wVar.w() - 2;
            this.c = 2;
            return 0;
        }
        if (i11 != 2) {
            if (i11 != 4) {
                if (i11 != 5) {
                    if (i11 == 6) {
                        return -1;
                    }
                    throw new IllegalStateException();
                }
                if (this.i == null || lVar != this.h) {
                    this.h = lVar;
                    this.i = new f2.c(lVar, this.f);
                }
                w3.l lVar2 = this.j;
                lVar2.getClass();
                int d = lVar2.d(this.i, nVar);
                if (d == 1) {
                    nVar.a += this.f;
                }
                return d;
            }
            long j11 = ((h) lVar).d;
            long j12 = this.f;
            if (j11 != j12) {
                nVar.a = j12;
                return 1;
            }
            h hVar = (h) lVar;
            if (!hVar.f(wVar.a, 0, 1, true)) {
                a();
                return 0;
            }
            hVar.f = 0;
            if (this.j == null) {
                this.j = new w3.l(0);
            }
            f2.c cVar2 = new f2.c(lVar, this.f);
            this.i = cVar2;
            this.j.getClass();
            if (!j.i(cVar2, false, false)) {
                a();
                return 0;
            }
            w3.l lVar3 = this.j;
            long j13 = this.f;
            m mVar = this.b;
            mVar.getClass();
            lVar3.q = new f2.c(j13, mVar, 11);
            h4.b bVar2 = this.g;
            bVar2.getClass();
            b(bVar2);
            this.c = 5;
            return 0;
        }
        if (this.d == 65505) {
            w wVar2 = new w(this.e);
            ((h) lVar).c(wVar2.a, 0, this.e, false);
            if (this.g == null && "http://ns.adobe.com/xap/1.0/".equals(wVar2.m()) && (m10 = wVar2.m()) != null) {
                long j14 = ((h) lVar).c;
                if (j14 != -1) {
                    try {
                        cVar = d.a(m10);
                    } catch (t1 | NumberFormatException | XmlPullParserException e10) {
                        f5.a.L("MotionPhotoXmpParser", "Ignoring unexpected XMP metadata", e10);
                        cVar = null;
                    }
                    if (cVar != null && (i10 = (l0Var = (l0) cVar.c).d) >= 2) {
                        int i12 = i10 - 1;
                        long j15 = -1;
                        long j16 = -1;
                        long j17 = -1;
                        long j18 = -1;
                        boolean z10 = false;
                        while (i12 >= 0) {
                            b bVar3 = (b) l0Var.get(i12);
                            boolean equals = "video/mp4".equals(bVar3.a) | z10;
                            if (i12 == 0) {
                                j14 -= bVar3.d;
                                j10 = 0;
                            } else {
                                j10 = j14 - bVar3.c;
                            }
                            long j19 = j10;
                            long j20 = j14;
                            j14 = j19;
                            if (equals && j14 != j20) {
                                j18 = j20 - j14;
                                j17 = j14;
                                equals = false;
                            }
                            if (i12 == 0) {
                                j15 = j14;
                                j16 = j20;
                            }
                            i12--;
                            z10 = equals;
                        }
                        if (j17 != -1 && j18 != -1 && j15 != -1 && j16 != -1) {
                            bVar = new h4.b(j15, j16, cVar.b, j17, j18);
                            this.g = bVar;
                            if (bVar != null) {
                                this.f = bVar.d;
                            }
                        }
                    }
                }
                bVar = null;
                this.g = bVar;
                if (bVar != null) {
                }
            }
        } else {
            ((h) lVar).t(this.e);
        }
        this.c = 0;
        return 0;
    }

    @Override // o3.k
    public final boolean e(l lVar) {
        h hVar = (h) lVar;
        w wVar = this.a;
        wVar.z(2);
        hVar.f(wVar.a, 0, 2, false);
        if (wVar.w() == 65496) {
            wVar.z(2);
            hVar.f(wVar.a, 0, 2, false);
            int w10 = wVar.w();
            this.d = w10;
            if (w10 == 65504) {
                wVar.z(2);
                hVar.f(wVar.a, 0, 2, false);
                hVar.b(wVar.w() - 2, false);
                wVar.z(2);
                hVar.f(wVar.a, 0, 2, false);
                this.d = wVar.w();
            }
            if (this.d == 65505) {
                hVar.b(2, false);
                wVar.z(6);
                hVar.f(wVar.a, 0, 6, false);
                if (wVar.s() == 1165519206 && wVar.w() == 0) {
                    return true;
                }
            }
        }
        return false;
    }

    @Override // o3.k
    public final void f(long j10, long j11) {
        if (j10 == 0) {
            this.c = 0;
            this.j = null;
        } else if (this.c == 5) {
            w3.l lVar = this.j;
            lVar.getClass();
            lVar.f(j10, j11);
        }
    }

    @Override // o3.k
    public final void i(m mVar) {
        this.b = mVar;
    }

    @Override // o3.k
    public final void release() {
        w3.l lVar = this.j;
        if (lVar != null) {
            lVar.getClass();
        }
    }
}
