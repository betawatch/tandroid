...
public abstract class ColorUtils {
    ...
    private static int compositeAlpha(int i, int i2) {
        return 255 - (((255 - i2) * (255 - i)) / NotificationCenter.didReplacedPhotoInMemCache);
    }
    ...
    private static int compositeComponent(int i, int i2, int i3, int i4, int i5) {
        ...
        return (((i * NotificationCenter.didReplacedPhotoInMemCache) * i2) + ((i3 * i4) * (255 - i2))) / (i5 * NotificationCenter.didReplacedPhotoInMemCache);
    }
    ...
    public static int calculateMinimumAlpha(int i, int i2, float f) {
        ...
        double d = f;
        if (calculateContrast(setAlphaComponent(i, NotificationCenter.didReplacedPhotoInMemCache), i2) < d) {
            return -1;
        }
        int i4 = 0;
    ...
    public static int HSLToColor(float[] fArr) {
        ...
        return Color.rgb(constrain(round, 0, NotificationCenter.didReplacedPhotoInMemCache), constrain(round2, 0, NotificationCenter.didReplacedPhotoInMemCache), constrain(round3, 0, NotificationCenter.didReplacedPhotoInMemCache));
    }
    ...
    public static int XYZToColor(double d, double d2, double d3) {
        ...
        return Color.rgb(constrain((int) Math.round((d4 > 0.0031308d ? (Math.pow(d4, 0.4166666666666667d) * 1.055d) - 0.055d : d4 * 12.92d) * 255.0d), 0, NotificationCenter.didReplacedPhotoInMemCache), constrain((int) Math.round((d5 > 0.0031308d ? (Math.pow(d5, 0.4166666666666667d) * 1.055d) - 0.055d : d5 * 12.92d) * 255.0d), 0, NotificationCenter.didReplacedPhotoInMemCache), constrain((int) Math.round((d6 > 0.0031308d ? (Math.pow(d6, 0.4166666666666667d) * 1.055d) - 0.055d : 12.92d * d6) * 255.0d), 0, NotificationCenter.didReplacedPhotoInMemCache));
    }
