package l4;

import android.graphics.Paint;
import android.media.MediaCodec;
import java.nio.ByteBuffer;
import java.util.ArrayList;
import java.util.Arrays;
import org.telegram.messenger.AndroidUtilities;
import org.telegram.messenger.MediaDataController;
import org.telegram.tgnet.TLObject;
import org.telegram.ui.Components.xb0;

/* compiled from: r8-map-id-53ae6996d745fb61649afae8ef429049dda227e2aa02488fda2c3b459d1c2b94 */
/* loaded from: classes.dex */
public final class w0 {
    public final int a;
    public long b;
    public final Object c;
    public final Object d;
    public Object e;
    public Object f;
    public Object g;

    public w0(com.google.android.exoplayer2.upstream.r rVar) {
        this.c = rVar;
        int i10 = rVar.b;
        this.a = i10;
        this.d = new f5.w(32);
        v0 v0Var = new v0(0L, i10);
        this.e = v0Var;
        this.f = v0Var;
        this.g = v0Var;
    }

    public static v0 d(v0 v0Var, long j10, ByteBuffer byteBuffer, int i10) {
        while (j10 >= v0Var.b) {
            v0Var = (v0) v0Var.d;
        }
        while (i10 > 0) {
            int min = Math.min(i10, (int) (v0Var.b - j10));
            com.google.android.exoplayer2.upstream.a aVar = (com.google.android.exoplayer2.upstream.a) v0Var.c;
            byteBuffer.put(aVar.a, ((int) (j10 - v0Var.a)) + aVar.b, min);
            i10 -= min;
            j10 += min;
            if (j10 == v0Var.b) {
                v0Var = (v0) v0Var.d;
            }
        }
        return v0Var;
    }

    public static v0 e(v0 v0Var, long j10, byte[] bArr, int i10) {
        while (j10 >= v0Var.b) {
            v0Var = (v0) v0Var.d;
        }
        int i11 = i10;
        while (i11 > 0) {
            int min = Math.min(i11, (int) (v0Var.b - j10));
            com.google.android.exoplayer2.upstream.a aVar = (com.google.android.exoplayer2.upstream.a) v0Var.c;
            System.arraycopy(aVar.a, ((int) (j10 - v0Var.a)) + aVar.b, bArr, i10 - i11, min);
            i11 -= min;
            j10 += min;
            if (j10 == v0Var.b) {
                v0Var = (v0) v0Var.d;
            }
        }
        return v0Var;
    }

    public static v0 f(v0 v0Var, m3.i iVar, y0 y0Var, f5.w wVar) {
        if (iVar.getFlag(TLObject.FLAG_30)) {
            long j10 = y0Var.a;
            int i10 = 1;
            wVar.z(1);
            v0 e10 = e(v0Var, j10, wVar.a, 1);
            long j11 = j10 + 1;
            byte b10 = wVar.a[0];
            boolean z10 = (b10 & 128) != 0;
            int i11 = b10 & Byte.MAX_VALUE;
            m3.d dVar = iVar.a;
            byte[] bArr = dVar.a;
            if (bArr == null) {
                dVar.a = new byte[16];
            } else {
                Arrays.fill(bArr, (byte) 0);
            }
            v0Var = e(e10, j11, dVar.a, i11);
            long j12 = j11 + i11;
            if (z10) {
                wVar.z(2);
                v0Var = e(v0Var, j12, wVar.a, 2);
                j12 += 2;
                i10 = wVar.w();
            }
            int[] iArr = dVar.d;
            if (iArr == null || iArr.length < i10) {
                iArr = new int[i10];
            }
            int[] iArr2 = dVar.e;
            if (iArr2 == null || iArr2.length < i10) {
                iArr2 = new int[i10];
            }
            if (z10) {
                int i12 = i10 * 6;
                wVar.z(i12);
                v0Var = e(v0Var, j12, wVar.a, i12);
                j12 += i12;
                wVar.C(0);
                for (int i13 = 0; i13 < i10; i13++) {
                    iArr[i13] = wVar.w();
                    iArr2[i13] = wVar.u();
                }
            } else {
                iArr[0] = 0;
                iArr2[0] = y0Var.b - ((int) (j12 - y0Var.a));
            }
            o3.v vVar = (o3.v) y0Var.c;
            int i14 = f5.d0.a;
            byte[] bArr2 = vVar.b;
            byte[] bArr3 = dVar.a;
            int i15 = vVar.a;
            int i16 = vVar.c;
            int i17 = vVar.d;
            dVar.f = i10;
            dVar.d = iArr;
            dVar.e = iArr2;
            dVar.b = bArr2;
            dVar.a = bArr3;
            dVar.c = i15;
            dVar.g = i16;
            dVar.h = i17;
            MediaCodec.CryptoInfo cryptoInfo = dVar.i;
            cryptoInfo.numSubSamples = i10;
            cryptoInfo.numBytesOfClearData = iArr;
            cryptoInfo.numBytesOfEncryptedData = iArr2;
            cryptoInfo.key = bArr2;
            cryptoInfo.iv = bArr3;
            cryptoInfo.mode = i15;
            if (f5.d0.a >= 24) {
                m3.c cVar = dVar.j;
                cVar.getClass();
                m3.c.a(cVar, i16, i17);
            }
            long j13 = y0Var.a;
            int i18 = (int) (j12 - j13);
            y0Var.a = j13 + i18;
            y0Var.b -= i18;
        }
        if (!iVar.hasSupplementalData()) {
            iVar.b(y0Var.b);
            return d(v0Var, y0Var.a, iVar.b, y0Var.b);
        }
        wVar.z(4);
        v0 e11 = e(v0Var, y0Var.a, wVar.a, 4);
        int u10 = wVar.u();
        y0Var.a += 4;
        y0Var.b -= 4;
        iVar.b(u10);
        v0 d = d(e11, y0Var.a, iVar.b, u10);
        y0Var.a += u10;
        int i19 = y0Var.b - u10;
        y0Var.b = i19;
        ByteBuffer byteBuffer = iVar.e;
        if (byteBuffer == null || byteBuffer.capacity() < i19) {
            iVar.e = ByteBuffer.allocate(i19);
        } else {
            iVar.e.clear();
        }
        return d(d, y0Var.a, iVar.e, y0Var.b);
    }

    public void a(v0 v0Var) {
        if (((com.google.android.exoplayer2.upstream.a) v0Var.c) == null) {
            return;
        }
        com.google.android.exoplayer2.upstream.r rVar = (com.google.android.exoplayer2.upstream.r) this.c;
        synchronized (rVar) {
            v0 v0Var2 = v0Var;
            while (v0Var2 != null) {
                try {
                    com.google.android.exoplayer2.upstream.a[] aVarArr = rVar.f;
                    int i10 = rVar.e;
                    rVar.e = i10 + 1;
                    com.google.android.exoplayer2.upstream.a aVar = (com.google.android.exoplayer2.upstream.a) v0Var2.c;
                    aVar.getClass();
                    aVarArr[i10] = aVar;
                    rVar.d--;
                    v0Var2 = (v0) v0Var2.d;
                    if (v0Var2 == null || ((com.google.android.exoplayer2.upstream.a) v0Var2.c) == null) {
                        v0Var2 = null;
                    }
                } catch (Throwable th2) {
                    throw th2;
                }
            }
            rVar.notifyAll();
        }
        v0Var.c = null;
        v0Var.d = null;
    }

    public void b(long j10) {
        v0 v0Var;
        if (j10 == -1) {
            return;
        }
        while (true) {
            v0Var = (v0) this.e;
            if (j10 < v0Var.b) {
                break;
            }
            com.google.android.exoplayer2.upstream.r rVar = (com.google.android.exoplayer2.upstream.r) this.c;
            com.google.android.exoplayer2.upstream.a aVar = (com.google.android.exoplayer2.upstream.a) v0Var.c;
            synchronized (rVar) {
                com.google.android.exoplayer2.upstream.a[] aVarArr = rVar.f;
                int i10 = rVar.e;
                rVar.e = i10 + 1;
                aVarArr[i10] = aVar;
                rVar.d--;
                rVar.notifyAll();
            }
            v0 v0Var2 = (v0) this.e;
            v0Var2.c = null;
            v0 v0Var3 = (v0) v0Var2.d;
            v0Var2.d = null;
            this.e = v0Var3;
        }
        if (((v0) this.f).a < v0Var.a) {
            this.f = v0Var;
        }
    }

    public int c(int i10) {
        com.google.android.exoplayer2.upstream.a aVar;
        v0 v0Var = (v0) this.g;
        if (((com.google.android.exoplayer2.upstream.a) v0Var.c) == null) {
            com.google.android.exoplayer2.upstream.r rVar = (com.google.android.exoplayer2.upstream.r) this.c;
            synchronized (rVar) {
                try {
                    int i11 = rVar.d + 1;
                    rVar.d = i11;
                    int i12 = rVar.e;
                    if (i12 > 0) {
                        com.google.android.exoplayer2.upstream.a[] aVarArr = rVar.f;
                        int i13 = i12 - 1;
                        rVar.e = i13;
                        aVar = aVarArr[i13];
                        aVar.getClass();
                        rVar.f[rVar.e] = null;
                    } else {
                        com.google.android.exoplayer2.upstream.a aVar2 = new com.google.android.exoplayer2.upstream.a(new byte[rVar.b], 0);
                        com.google.android.exoplayer2.upstream.a[] aVarArr2 = rVar.f;
                        if (i11 > aVarArr2.length) {
                            rVar.f = (com.google.android.exoplayer2.upstream.a[]) Arrays.copyOf(aVarArr2, aVarArr2.length * 2);
                        }
                        aVar = aVar2;
                    }
                } catch (Throwable th2) {
                    throw th2;
                }
            }
            v0 v0Var2 = new v0(((v0) this.g).b, this.a);
            v0Var.c = aVar;
            v0Var.d = v0Var2;
        }
        return Math.min(i10, (int) (((v0) this.g).b - this.b));
    }

    public w0(xb0 xb0Var) {
        this.d = new ArrayList(50);
        this.e = new ArrayList(50);
        Paint paint = new Paint(1);
        this.f = paint;
        this.a = MediaDataController.MAX_LINKS_COUNT;
        this.c = xb0Var;
        paint.setStrokeWidth(AndroidUtilities.dp(1.33f));
    }
}
